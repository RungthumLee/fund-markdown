...
def main() -> None:
    ...
    o += ["---", "", "## 5. ข้อจำกัดที่ต้องรู้ก่อนใช้ข้อมูล", "",
          ...
          "NAV เก็บ 5 ปี (`NAV_YEARS`) เป็นเพดานที่เลือกเอง — ต้นทางให้ถึงวันจัดตั้ง · "
          "พอร์ตเก็บ 12 ไตรมาส ซึ่งเป็น**เพดานของ API เอง** · "
          "กองที่อายุน้อยกว่านั้นได้เท่าที่มี — ดู [[decisions|DEC-002]]", "",
          "### 5.2b NAV ไม่ต่อเนื่องทุกกอง — และชื่อกองที่เปลี่ยนไม่มีประวัติ",
          "ต้นทางขาดหายทั้งตลาดช่วง **1–12 พ.ย. 2024** (ยืนยันด้วยการเรียก API "
          "ตรง ๆ: ทั้งช่วงมี 21 แถว) กระทบ ~1,500 series · "
          "กองที่ประกาศ NAV รายเดือน/รายสัปดาห์มีอยู่จริง (51 series) ไม่ใช่ข้อมูลขาด · "
          "และเมื่อกองเปลี่ยนชื่อ `profiles` เก็บแค่ชื่อปัจจุบัน ร่องรอยเดียวคือ "
          "`fund_class_name` ในชุด NAV — ต่อ series ให้แล้วตามเกณฑ์ที่ตรวจสอบได้ "
          "ดู [[nav-continuity|NAV Continuity Report]] และ "
          "[[../../vault/Concepts/การเปลี่ยนชื่อกองทุนกับ NAV|แนวคิด]]", "",
