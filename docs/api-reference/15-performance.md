...
operation_id: getFactsheetPerformance
endpoint: /v2/fund/factsheet/performance
dataset: fs_performance
...
> Operation id: `getFactsheetPerformance`
...
This API provides the historical performance as reported for each fund and share class in the Fund Factsheet by Asset Management Companies (AMCs).
...
```http
GET https://api.sec.or.th/v2/fund/factsheet/performance?page_size=100
...
| `items[].performance_type_desc` | string | คำอธิบายประเภทผลตอบแทน |
| `items[].reference_period` | string | หมุดเวลาและปีย้อนหลัง |
| `items[].performance_value` | string | ผลการดำเนินงานย้อนหลังของกองทุน (8.3) |
...
- Dataset: `data/raw/fs_performance.jsonl`
- ดึงข้อมูล: `python scripts/harvest.py fs_performance`
