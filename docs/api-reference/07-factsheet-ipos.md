---
title: IPO Offering Period
operation_id: getFactsheetIPO
endpoint: /v2/fund/factsheet/ipos
dataset: fs_ipos
...
This API provides information on the Initial Public Offering (IPO) period of mutual funds, based on the Fund Fact Sheet reported by Asset Management Companies (AMCs).
...
```http
GET https://api.sec.or.th/v2/fund/factsheet/ipos?page_size=100
...
| `start_date` | string | no | ระบุวันที่เริ่มต้นที่ factsheet มีผล (รูปแบบ: YYYY-MM-DD) ระบบจะคืนข้อมูลที่มี start_date มากกว่าหรือเท่ากับค่าที่ระบุ |
...
| `items[].first_sell_start_date` | string | วันเริ่ม IPO |
| `items[].first_sell_end_date` | string | วันสิ้นสุด IPO |
 ...
 "items": [
  {
   ...
   "first_sell_start_date": "14/01/2552",
   "first_sell_end_date": "20/01/2552",
...
- Dataset: `data/raw/fs_ipos.jsonl`
- ดึงข้อมูล: `python scripts/harvest.py fs_ipos`
