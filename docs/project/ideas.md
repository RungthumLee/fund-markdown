...
### 5.5 ก่อนสเกลจริง — ✅ ทดสอบแล้ว (2026-08-28)

รันจริงด้วย `python scripts/probe_history.py` (3 กองต่างรุ่น · 463 API calls ·
ผลดิบที่ `data/processed/history_probe.json`)

| กอง | จัดตั้ง | NAV ที่ API ให้จริง | งวด portfolio ที่ให้ |
|---|---|---|---|
| K-FIXED | 1995-03-17 | **1995–2026 (32 ปี = ถึงวันจัดตั้ง)** | 202309–202606 (12 งวด) |
| 1AMSET50 | 2003-10-14 | 2003–2026 (24 ปี = ถึงวันจัดตั้ง) | 202309–202606 (12 งวด) |
| A-RING | 2025-11-10 | 2025–2026 (เท่าที่มี) | 202512–202606 (3 งวด) |

**ผล 3 ข้อ**
1. **NAV ไม่มีเพดานฝั่ง API** — ปีก่อนจัดตั้งคืน 0 แถว → ขอบเขตคืออายุกอง
   ยืนยันกฎ `min(5 ปี, ตั้งแต่จัดตั้ง)` ใช้ได้จริง
2. **Portfolio มีเพดานจริงที่งวด 202309** — กองอายุ 32 ปีกับ 24 ปีได้งวดแรกสุด
   **เท่ากันเป๊ะ** → เป็น retention ของต้นทาง = **12 ไตรมาส (3 ปี)**
   บังเอิญตรงกับ policy §5.4 พอดี → `MAX_HOLDING_QUARTERS=12` คือ**ทั้งหมดที่มี**
3. **Rate limit: burst ไม่เจอ แต่งานจริงเจอ** — ยิง 40 call ไม่เจอ 429 เลยแม้ที่
   73 req/s (8 threads) **แต่พอ backfill จริงด้วย 4 workers เจอ 429 ภายใน 3 นาที**
   → limiter เป็น **quota แบบต่อเนื่อง** ที่ burst test มองไม่เห็น ·
   ยิงเรียงต่อกัน delay 0.12 = 429 เป็นศูนย์ ~120 KB/s → **ใช้ `--workers=1`**
   ดู [[outstanding|OUT-002]]

**ต้นทุน backfill ที่วัดจากของจริง:** ดึงแบบ bulk ทั้งตลาด NAV 5 ปี ≈ **1.05 GB /
~5.8 ล้านแถว** (4,663 share class × ~1,250 วันทำการ) ที่ ~120 KB/s แบบยิงเรียง
= **~2.5 ชั่วโมง** (เร่งด้วยการขนานไม่ได้ ดูข้อ 3) · portfolio 12 ไตรมาส ≈ 280 MB ≈ 40 นาที
— ทำครั้งเดียว หลังจากนั้น append รายวันถูกมาก

**ขั้นถัดไป:** ตั้ง `MAX_NAV_YEARS=5` / `MAX_HOLDING_QUARTERS=12` ใน
`scripts/harvest.py` แล้วรัน `--force nav out_portfolio` — ดู [[decisions|DEC-002]]
...
## 6. Correlation ระหว่างกอง เพื่อตรวจ tag/factor — **ทดสอบแล้ว ตัดสินว่าไม่ทำ** (2026-08-29)

> [!WARNING] อย่าเสนอซ้ำโดยไม่อ่านหัวข้อนี้ก่อน
> ไอเดียนี้ฟังดูดีมากและ **prototype ก็ทำได้จริง** แต่ทดสอบแล้วไม่คุ้ม — บันทึกไว้พร้อม
> ตัวเลข เพื่อไม่ให้รอบหน้าเสียเวลาคิดใหม่

### ไอเดียที่เสนอ
ใช้ correlation ของ NAV ระหว่างกอง ยืนยันว่า faceted tag / factor ที่เราติดถูกต้อง —
"ถ้ากอง A กับ B วิ่งเหมือนกันมากแต่ tag ต่างกัน แปลว่ามีอะไรผิด" · ต่อยอดเป็น
peer group ที่วัดได้ · หา factor ทางอ้อมจากเพื่อนบ้าน · จัดกลุ่มจาก correlation

### สิ่งที่วัดได้จาก prototype (1,814 กองที่มี NAV ≥250 วัน · 1.6 ล้านคู่ · ~1 นาที)
1. **correlation ดิบใช้ไม่ได้** — คู่ที่ r≥0.90 มี 45,493 คู่ และ clustering ได้ก้อนเดียว
   ขนาด 460 กอง เพราะกองหุ้นไทยวิ่งตาม SET เหมือนกันหมด (วัด market beta ไม่ใช่ความเหมือน)
2. **ถอดตลาดออกแล้วดีขึ้นจริง** — regress กับ SET + S&P500 แล้วดู residual:
   คู่ r≥0.90 เหลือ 3,910 (−91%) · SET50 index ข้าม บลจ. ยังอยู่ที่ 0.997 (เหมือนกันจริง) ·
   SSB↔SF5 ตกจาก 0.975 เหลือ 0.655 (แค่ตลาดเดียวกัน)
3. **แต่คิวที่ได้ไม่ใช่ error** — สุ่ม 20 คู่จากคิว (residual r≥0.90 + facet ขัดกันจริง
   ไม่ใช่ tag ขาด · seed 42 · ตั้งเกณฑ์ก่อนดู) ผลตัดสิน:
   **error จริง 2 · ต้นทางต่างกันจริง 15 · ตัดสินไม่ได้ 3** → **2/17 = 12%**
   ต่ำกว่าเกณฑ์ที่ตั้งไว้ (>50%) มาก

### ทำไมถึงไม่คุ้ม
- **correlation สูงไม่ได้แปลว่า tag ควรเหมือนกัน** — `KFMTFIRMF` (duration/medium) กับ
  `KFLTGOVRMF` (duration/long) r=0.939 เพราะเป็นบอนด์ไทยทั้งคู่ ชื่อกองบอกเองว่าคนละอายุ
  tag ถูกทั้งคู่ · กองฝาแฝดที่ห่อภาษีคนละแบบก็ต่างกันที่ `struct` อย่างชอบธรรม
- **ของที่มันจะบอก มีทางตรงกว่าอยู่แล้ว** — พอร์ตเดียวกัน→`feeder_master`/look-through ·
  กลุ่มเดียวกัน→AIMC peer_group · ดัชนีเดียวกัน→`fs_benchmarks`
- **เสี่ยงเรื่องการตีความ** — "กอง A กับ B correlation 0.97" เป็นตัวเลขที่ถูกอ่านเป็น
  "สับกองได้" ง่ายที่สุดในคลัง ใกล้เส้น §0 โดยผลตอบแทนไม่คุ้ม
- **ค่าดูแล** — layer + รายงานที่ต้อง regenerate ทุกรอบ และคิว 3,074 คู่ที่ 88% เป็นของถูก
  = รายงานที่ไม่มีใครไล่จนจบ

### สิ่งที่ได้จริงจากการทดสอบ (คุ้มค่าที่ทำ prototype)
เจอบั๊ก 2 ตัว — **แต่ทั้งคู่หาเจอด้วยการเทียบ tag กับ field ต้นทางของมันเอง
ไม่ต้องใช้ correlation เลย และวิธีนั้นระบุตัวได้ครบโดยไม่มี false positive**:
[[issues|ISS-043]] fx 230 กอง · [[issues|ISS-044]] struct 16 กอง

### บทเรียนที่ใช้ต่อได้
> **ตรวจ tag ให้เทียบกับ field ต้นทางของตัวมันเอง ไม่ใช่เทียบกับกองอื่น** —
> การเทียบกับกองอื่นให้สัญญาณที่ต้องมาตีความอีกชั้น ส่วนการเทียบกับต้นทางให้คำตอบ
> ที่ระบุตัวได้และอธิบายได้ว่าผิดตรงไหน (ตระกูลเดียวกับ ISS-009: ตัวเลขที่ดูน่าเชื่อ
> อันตรายกว่าไม่มีตัวเลข)

### ถ้าจะรื้อฟื้น ต้องมีอะไรใหม่
- มีแหล่งข้อมูล "กองไหนควรอยู่กลุ่มเดียวกัน" ที่เชื่อถือได้มาเทียบ (ground truth)
- หรือเปลี่ยนโจทย์จาก "ตรวจ tag" เป็น "ตอบผู้ใช้ว่าฉันถือ 2 กองนี้ซ้ำกันไหม" ซึ่ง
  `portfolio-overlap` ตอบจาก holdings ได้ตรงกว่าอยู่แล้ว
- prototype เก็บไว้ที่ประวัติ session นี้ · ถ้าจะรันซ้ำ ใช้ `nav_history.json` +
  `factor_series.json` + pandas ~40 บรรทัด
