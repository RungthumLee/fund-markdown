---
title: "BCAP-XDIGI"
proj_id: M0382_2566
regis_id: "MF0174_2567"
abbr: "BCAP-XDIGI"
name_th: "กองทุนเปิดบีแคป เน็กซ์ เจน ดิจิตอล ไลฟ์สไตล์"
name_en: "BCAP NEXT GEN Digital Lifestyle Fund"
amc: "บริษัทหลักทรัพย์จัดการกองทุนรวม  บัวหลวง จำกัด"
amc_id: "C0000000329"
policy: "ตราสารทุน"
risk_spectrum: "6"
management_style: "AM"
retail_type: "R"
invest_country_flag: "1"
init_date: "2024-01-31"
regis_date: "2024-04-04"
class_count: 1
has_factsheet: true
ter_retail: 1.48
perf_1y: -2.98
nav: 10.4828
nav_date: "2026-08-25"
fund_size: 70507282.3
holdings_count: 13
top10_pct_nav: 98.98
market_countries: ["สหรัฐฯ"]
country_top: "สหรัฐฯ"
country_coverage_pct: 82.4
peer_group: "Global Equity"
tags: [fund, sec-data, asset/equity, risk/high, liquidity/t3, conc/ultra-concentrated, fx/discretionary, struct/direct, style/active, use/accumulate]
---

# BCAP-XDIGI

**กองทุนเปิดบีแคป เน็กซ์ เจน ดิจิตอล ไลฟ์สไตล์**  
_BCAP NEXT GEN Digital Lifestyle Fund_

บลจ. [[บริษัทหลักทรัพย์จัดการกองทุนรวม บัวหลวง จำกัด]] · นโยบาย [[ตราสารทุน]] · ความเสี่ยงระดับ **6**

> [!abstract] สรุปย่อ
> - **รหัสโครงการ:** `M0382_2566`
> - **ประเภท:** ตราสารทุน · Active management
> - **ผู้ลงทุน:** ผู้ลงทุนทั่วไป
> - **ต่างประเทศ:** เน้นลงทุนต่างประเทศ
> - **จัดตั้ง:** 2024-01-31 · **จดทะเบียน:** 2024-04-04
> - **ชนิดหน่วยลงทุน:** 1 class
> - **NAV 10.4828 (2026-08-25)**

> [!tip] อ่านง่าย ๆ
> กองหุ้น · ความเสี่ยงสูง (6/8) · เหมาะกับสะสมระยะยาว · ขายคืนแล้วได้เงินภายใน T+3 · ลงทุนต่างประเทศ (สหรัฐฯ) · ป้องกันค่าเงินตามดุลยพินิจผู้จัดการ · บริหารเชิงรุก (active) · พอร์ตถือราว 13 หลักทรัพย์ · ค่าธรรมเนียมรวมที่รายย่อยจ่ายจริงราว 1.48%/ปี

## 1. ข้อมูลทั่วไป

| รายการ | ค่า |
|---|---|
| เลขที่โครงการ (proj_id) | `M0382_2566` |
| เลขที่จดทะเบียน (regis_id) | MF0174_2567 |
| ชื่อย่อ | BCAP-XDIGI |
| ชื่อไทย | กองทุนเปิดบีแคป เน็กซ์ เจน ดิจิตอล ไลฟ์สไตล์ |
| ชื่ออังกฤษ | BCAP NEXT GEN Digital Lifestyle Fund |
| บลจ. | [[บริษัทหลักทรัพย์จัดการกองทุนรวม บัวหลวง จำกัด]] |
| สถานะ | Registered |
| วันจัดตั้ง | 2024-01-31 |
| วันจดทะเบียน | 2024-04-04 |
| ประเภทตามนโยบาย | ตราสารทุน |
| กลยุทธ์บริหาร | AM — Active management |
| ลักษณะโครงการ | R — ผู้ลงทุนทั่วไป |
| ความเสี่ยงต่างประเทศ | เน้นลงทุนต่างประเทศ |

### นโยบายป้องกันความเสี่ยงอัตราแลกเปลี่ยน

ดุลยพินิจ (dynamic hedging) (0%-105% ของมูลค่าความเสี่ยง)

## 2. ชนิดหน่วยลงทุน (Share Class)

| Class | ISIN | สิทธิประโยชน์ภาษี | รายละเอียด |
|---|---|---|---|
| main | - | - | - |

## 3. นโยบายการลงทุน

กองทุนมีนโยบายการลงทุนในหน่วยลงทุนของกองทุนรวมตราสารทุนต่างประเทศ โดยกองทุนปลายทางมีการกระจายการลงทุนในหลักทรัพย์หรือตราสารของบริษัทที่ได้ประโยชน์จากการประกอบธุรกิจที่เกี่ยวข้องกับเทคโนโลยีดิจิทัลรูปแบบใหม่ที่จะตอบสนองแนวโน้มพฤติกรรมการใช้ชีวิตแห่งอนาคต เช่น Cyber Security, Digital Commerce, Financial Technology, Digital Education, Digital Entertainment & Leisure, Remote Workplace, Smart City and Future Mobility เป็นต้น 
กองทุนจะลงทุนในหน่วยลงทุนของกองทุนรวมตราสารทุนต่างประเทศข้างต้น ตั้งแต่ 2 กองทุนขึ้นไป เพื่อให้มี net exposure โดยเฉลี่ยในรอบปีบัญชีไม่น้อยกว่าร้อยละ 80 ของมูลค่าทรัพย์สินสุทธิของกองทุน โดยมีสัดส่วนการลงทุนในแต่ละกองทุนโดยเฉลี่ยในรอบปีบัญชีกองทุนละไม่เกินร้อยละ 79 ของมูลค่าทรัพย์สินสุทธิของกองทุน ซึ่งสัดส่วนการลงทุนในหน่วยลงทุนของแต่ละกองทุนจะขึ้นอยู่กับดุลยพินิจของผู้จัดการกองทุน โดยสามารถปรับเปลี่ยนได้ตามสภาวะการลงทุนหรือการคาดการณ์สภาวะการลงทุนในแต่ละขณะ ทั้งนี้ กองทุนจะนำเงินไปลงทุนในต่างประเทศโดยเฉลี่ยในรอบปีบัญชีไม่น้อยกว่าร้อยละ 80 ของมูลค่าทรัพย์สินสุทธิของกองทุน
สำหรับเงินลงทุนส่วนที่เหลือ อาจพิจารณาลงทุนในหลักทรัพย์หรือทรัพย์สินอื่น หรือการหาดอกผลโดยวิธีอื่นอย่างใดอย่างหนึ่งหรือหลายอย่างซึ่งไม่ขัดต่อกฎหมาย หรือที่สำนักงาน ก.ล.ต. ให้ความเห็นชอบให้ลงทุนได้
กองทุนมีเป้าหมายในการลงทุนเพื่อให้ผลประกอบการสูงกว่าดัชนีชี้วัด (Active Management) โดยมีกระบวนการคัดเลือกหลักทรัพย์ โดยพิจารณาจาก 2 ปัจจัย ดังนี้
1. การวิเคราะห์แบบ Top Down Theme Allocation: เพื่อจัดสรรน้ำหนักการลงทุนในแต่ละกลุ่มอุตสาหกรรม/ธีมการลงทุน ที่มีแนวโน้มการเติบโตได้ดีในระยะยาว โดยพิจารณาจากปัจจัยเศรษฐกิจมหาภาค เช่น แนวโน้มอัตราการเจริญเติบโตของเศรษฐกิจและอุตสาหกรรมที่ไปลงทุน นโยบายสนับสนุนของทางภาครัฐฯ นโยบายการเงิน การคลัง อัตราเงินเฟ้อ เป็นต้น 
2. การวิเคราะห์แบบ Bottom Up Selection: ซึ่งให้ความสำคัญในการเลือกหลักทรัพย์หรือหน่วยลงทุนของกองทุนรวมต่างประเทศ เช่น หน่วย CIS และ/หรือหน่วยลงทุนของกองทุน อีทีเอฟ (ETF) ต่างประเทศที่ไปลงทุน โดยพิจารณาจากปัจจัยพื้นฐานรอบด้าน เช่น Index Methodology และ Investment Process ของหน่วยลงทุนที่ไปลงทุน, รูปแบบการดำเนินธุรกิจ (Business Model), ความสามารถในการทำกำไรและรายได้ (Profitability), ความสามารถในการแข่งขัน (Competitive Advantage), อัตราส่วนทางการเงิน รวมไปถึง Valuation ของกลุ่มบริษัทที่ไปลงทุน เป็นต้น อีกทั้งมีการวิเคราะห์ขนาดกองทุนและสภาพคล่องของกองทุนที่ไปลงทุนเพื่อลดความเสี่ยงด้านสภาพคล่องของหลักทรัพย์หรือหน่วยลงทุนของกองทุนรวมต่างประเทศที่ไปลงทุน
กองทุนอาจลงทุนในสัญญาซื้อขายล่วงหน้า (Derivative) เพื่อเพิ่มประสิทธิภาพการบริหารการลงทุน (Efficient Portfolio Management) โดยอาจลงทุนในสัญญาซื้อขายล่วงหน้าที่มีสินทรัพย์อ้างอิง ได้แก่ ทรัพย์สินต่าง ๆ ที่กองทุนสามารถลงทุนได้ อัตราดอกเบี้ย อัตราแลกเปลี่ยน เครดิต สินค้าโภคภัณฑ์ ดัชนีกลุ่มสินค้าโภคภัณฑ์ ดัชนีเงินเฟ้อ ดัชนีอ้างอิงอื่น ๆ ตามดุลยพินิจของบลจ. และอาจป้องกันความเสี่ยงด้านอัตราแลกเปลี่ยน (Hedging) ตามดุลยพินิจของผู้จัดการกองทุน นอกจากนี้ กองทุนอาจลงทุนในตราสารที่มีลักษณะของสัญญาซื้อขายล่วงหน้าแฝง (Structured Note) หลักทรัพย์ที่ไม่มีการซื้อขายในตลาดหลักทรัพย์ (Unlisted Securities) ตราสารที่มีอันดับความน่าเชื่อถือที่ตัวตราสารหรือที่ผู้ออกตราสารต่ำกว่าที่สามารถลงทุนได้ (Non-Investment Grade) และ/หรือตราสารที่ไม่ได้รับการจัดอันดับความน่าเชื่อถือที่ตัวตราสารและที่ผู้ออกตราสาร (Unrated Securities) 
บริษัทจัดการขอสงวนสิทธิที่จะนำเงินลงทุนบางส่วนหรือทั้งหมดมาลงทุนในประเทศไทย ทั้งนี้ เพื่อประโยชน์ของกองทุนโดยรวม เช่น ในกรณีที่มีความผิดปกติของตลาด หรือในกรณีที่มีการเปลี่ยนแปลงของภาวะตลาดหรือการคาดการณ์ภาวะตลาดทำให้การลงทุนในต่างประเทศขาดความเหมาะสมในทางปฏิบัติ หรือกรณีอื่นใดที่เกิดจากสาเหตุที่ไม่สามารถควบคุมได้ เช่น การเปลี่ยนแปลงของกฎหมาย หรือประกาศที่เกี่ยวข้อง หรือเกิดภาวะสงคราม เป็นต้น
บริษัทจัดการขอสงวนสิทธิเปลี่ยนแปลงประเภทและลักษณะพิเศษของกองทุนเป็นกองทุนรวมที่มีการลงทุนโดยตรง หรือ กองทุน รวมฟีดเดอร์ (Feeder Fund) หรือสามารถกลับมาเป็นกองทุนรวมหน่วยลงทุน (Fund of Funds) โดยเป็นไปตามหลักเกณฑ์ที่สำนักงานคณะกรรมการ ก.ล.ต. ประกาศกำหนดหรือให้ความเห็นชอบให้กองทุนลงทุนได้ ทั้งนี้ การเปลี่ยนแปลงประเภทและลักษณะพิเศษของกองทุนจะไม่ทำให้ระดับความเสี่ยงของการลงทุน (Risk Profile) เพิ่มขึ้น รวมทั้งประเภทและลักษณะพิเศษที่เปลี่ยนแปลงนั้นจะเป็นไปตามดุลยพินิจของผู้จัดกา …(ตัดทอน)

### ลักษณะเฉพาะของโครงการ

- `FF` กองทุนรวมหน่วยลงทุน (Fund of Funds)

## 4. ความเสี่ยง

**ระดับความเสี่ยง: 6 / 8** — 6 — ตราสารทุน

> ลงทุนในตราสารทุนเป็นหลักโดยเฉลี่ยในรอบปีไม่น้อยกว่า 80% ของ NAV

อ่านเพิ่ม: [[ระดับความเสี่ยงกองทุนรวม]]

### สถิติเชิงปริมาณ

| Class | Sharpe | Alpha | Beta | Max Drawdown | Tracking Error | Turnover | FX Hedging |
|---|---|---|---|---|---|---|---|
| main | 0.03 | -11.76 | 1.29 | -28.05% | - | 1.08 | - |

อ่านเพิ่ม: [[สถิติวัดผลกองทุน]]

## 5. ค่าธรรมเนียม

ดูคำอธิบายแต่ละประเภทที่ [[ค่าธรรมเนียมกองทุนรวม]]

### สรุปต่อชนิดหน่วยลงทุน (Share Class)

| ชนิดหน่วยลงทุน | ซื้อได้โดย | รวม: เก็บจริง (%) | รวม: เพดาน (%) | จัดการ: เก็บจริง (%) | ขาย (%) | รับซื้อคืน (%) |
|---|---|---|---|---|---|---|
| `main` | ⚪ ไม่ระบุ | 1.48 | 2.68 | 1.07 | 1.07 | - |

**เก็บจริง** = อัตราที่เรียกเก็บในรอบที่รายงาน · **เพดาน** = อัตราสูงสุดที่หนังสือชี้ชวนอนุญาต — สองค่านี้เทียบกันข้ามกองไม่ได้

อธิบายชนิดหน่วยลงทุนที่ [[ชนิดหน่วยลงทุน Share Class]] · [[../Indexes/compare-fees|เทียบกับกองอื่นในหมวดเดียวกัน]]

### ตาม Factsheet ล่าสุด (ทุกแถวที่รายงาน)

| Class | ประเภท | เพดาน (%) | เก็บจริง (%) |
|---|---|---|---|
| main | ค่าธรรมเนียมการรับซื้อคืนหน่วยลงทุน (Back-end Fee) | 1.07 | 0 |
| main | ค่าธรรมเนียมการโอนหน่วยลงทุน (Transfer Fee) | 0 | 0 |
| main | ค่าธรรมเนียมการสับเปลี่ยนหน่วยลงทุนออก (SWITCHING OUT) | 0 | 0 |
| main | ค่าธรรมเนียมและค่าใช้จ่ายรวมทั้งหมด (Total Fee and Expense) | 2.68 | 1.48 |
| main | ค่าธรรมเนียมการขายหน่วยลงทุน (Front-end Fee) | 1.07 | 1.07 |
| main | ค่าธรรมเนียมการจัดการ (Management Fee) | 1.2 | 1.07 |
| main | ค่าธรรมเนียมการสับเปลี่ยนหน่วยลงทุนเข้า (SWITCHING IN) | 0 | 0 |

### ตามหนังสือชี้ชวน (เพดานตามโครงการ)

| Class | ประเภท | อัตรา | หน่วย |
|---|---|---|---|
| main | ค่าธรรมเนียมการจัดการ (Management Fee) | 1.2 | ต่อปี ของมูลค่าทรัพย์สินทั้งหมดหักด้วยมูลค่าหนี้สินทั้งหมด ก่อนหักค่าธรรมเนียมการจัดการ ค่าธรรมเนียมผู้ดูแลผลประโยชน์ และค่าธรรมเนียมนายทะเบียนหน่วยลงทุน หรือค่าธรรมเนียมอื่น ๆ ที่มีการคำนวณในลักษณะเดียวกันกับค่าธรรมเนียมดังกล่าวข้างต้น |
| main | ค่าธรรมเนียมทั้งหมด (Total Fee) | 2.675 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมที่ปรึกษาการลงทุน (Investment Advisor Fee) | - | - |
| main | ค่าธรรมเนียมนายทะเบียนหน่วยลงทุน (Registrar Fee) | 0.1338 | ต่อปี ของมูลค่าทรัพย์สินทั้งหมดหักด้วยมูลค่าหนี้สินทั้งหมด ก่อนหักค่าธรรมเนียมการจัดการ ค่าธรรมเนียมผู้ดูแลผลประโยชน์ และค่าธรรมเนียมนายทะเบียนหน่วยลงทุน หรือค่าธรรมเนียมอื่น ๆ ที่มีการคำนวณในลักษณะเดียวกันกับค่าธรรมเนียมดังกล่าวข้างต้น |
| main | ค่าธรรมเนียมผู้จัดจำหน่ายหน่วยลงทุน (Distributor Fee) | - | - |
| main | ค่าธรรมเนียมผู้ดูแลผลประโยชน์ (Trustee Fee) | 0.107 | ต่อปี ของมูลค่าทรัพย์สินทั้งหมดหักด้วยมูลค่าหนี้สินทั้งหมด ก่อนหักค่าธรรมเนียมการจัดการ ค่าธรรมเนียมผู้ดูแลผลประโยชน์ และค่าธรรมเนียมนายทะเบียนหน่วยลงทุน หรือค่าธรรมเนียมอื่น ๆ ที่มีการคำนวณในลักษณะเดียวกันกับค่าธรรมเนียมดังกล่าวข้างต้น |
| main | ค่าธรรมเนียมอื่น ๆ (Other Fee) | 1.2343 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |

## 6. ผลการดำเนินงาน

### Class `main`

| รายการ | 3 เดือน | 6 เดือน | YTD | 1 ปี | ตั้งแต่จัดตั้ง | 2025 | 2024 |
|---|---|---|---|---|---|---|---|
| ผลตอบแทนกองทุนรวม | 14.75 | 9.79 | 6.33 | -2.98 | 2.42 | -5.48 | 5.17 |
| ผลตอบแทนตัวชี้วัด | 6.58 | 14.75 | 17.15 | 24.68 | 14.17 | 13.38 | 2.44 |
| ค่าเฉลี่ยในกลุ่มเดียวกัน | 5.49 | 13.08 | 15.51 | 25.33 | - | - | - |
| ความผันผวนของกองทุนรวม | 21.21 | 22.78 | 22.01 | 20.52 | 21.64 | - | - |
| ความผันผวนของตัวชี้วัด | 11.3 | 12.41 | 12.22 | 11.33 | 13.79 | - | - |

> ตัวเลขเป็น % ต่อปี · เทียบ **ผลตอบแทนกองทุนรวม** กับ **ผลตอบแทนตัวชี้วัด** และ **ค่าเฉลี่ยในกลุ่มเดียวกัน** เสมอ  
> ดู [[สถิติวัดผลกองทุน]] · ผลตอบแทนในอดีตไม่รับประกันอนาคต

### ดัชนีชี้วัด (Benchmark)

| # | ดัชนีชี้วัด | หมายเหตุ |
|---|---|---|
| 1 | ดัชนี MSCI ACWI Net Total Return USD | ปรับด้วยอัตราแลกเปลี่ยนเพื่อคำนวณผลตอบแทนเป็นสกุลเงินบาท ณ วันที่คำนวณผลตอบแทน |

## 7. พอร์ตการลงทุน

### การกระจายตามประเทศ (ตลาดจริงจากหลักทรัพย์)

| ประเทศ (ตลาดที่ซื้อขาย) | % ของกอง |
|---|---|
| สหรัฐฯ | 82.4 |

> ทะลุถึงหลักทรัพย์จริงได้ **82.4%** ของกอง (ส่วนที่เหลือ 17.6% ยังไม่ทะลุ) · ที่มา: ISIN ของหลักทรัพย์ในพอร์ต (ISO 6166)

### ⚖️ ปัจจัยที่กระทบกอง (สองด้าน)

> [!INFO] ปัจจัยที่กองนี้**ไว**ต่อ — อ่านจากหมวด/ประเทศ/โครงสร้างที่ถือจริง
> แสดง**ทั้งสองด้าน** (ขึ้น=โอกาส · ลง=ความเสี่ยง) · **ไม่ใช่การพยากรณ์** ว่าจะไปทางไหน · [กรอบข้อมูลอ้างอิง](../../docs/project/ideas.md)

| ปัจจัย | หมวด | ถ้าขึ้น ▲ | ถ้าลง ▼ | จาก |
|---|---|---|---|---|
| ความเสี่ยงเฉพาะตัว (single-name/theme) | โครงสร้าง | ถูกทาง→ upside แรงกว่ากองกระจาย | ผิดทาง→ downside แรงกว่ากองกระจาย | โครงสร้างกอง |
| ดอกเบี้ยเฟด / เศรษฐกิจสหรัฐ | Macro | เศรษฐกิจแข็งแรง/เฟดผ่อนคลาย | เฟดขึ้นดอกเบี้ย/เศรษฐกิจชะลอ | ประเทศ: สหรัฐฯ |


### 📊 เคลื่อนไหวสัมพันธ์กับอะไร (correlation อดีต)

| ปัจจัย | correlation | ทิศทาง | จำนวนวัน |
|---|---|---|---|
| หุ้นสหรัฐ (S&P 500) | +0.76 | ไปทางเดียวกัน | 476 |
| หุ้นตลาดเกิดใหม่ (EM) | +0.58 | ไปทางเดียวกัน | 476 |

> **วัดจาก NAV รายวัน** ช่วง 2024-04-04..2026-08-25 (ที่มา: NAV × Yahoo) · +1 = ไปทางเดียวกัน · −1 = สวนทาง
> [!warning] เป็น**ความสัมพันธ์ที่วัดจากอดีตในช่วงข้างต้น** — **ไม่ใช่การพยากรณ์** · correlation ไม่ใช่สาเหตุ และ**ไม่นิ่ง** (มักพุ่งเข้า 1 ตอนตลาดวิกฤต)
### การจัดสรรสินทรัพย์ (จาก Factsheet)

| สินทรัพย์ | สัดส่วน (%) |
|---|---|
| หน่วยลงทุน | 98.86 |
| เงินฝากธนาคาร | 1.8 |

### 5 อันดับแรกที่ลงทุน

| # | หลักทรัพย์ | สัดส่วน (%) |
|---|---|---|
| 1 | FIRST TRUST NASDAQ CYBERSECU (CIBR US) | 32.97 |
| 2 | GLOBAL X SOCIAL MEDIA ETF (SOCL US) | 17.75 |
| 3 | EMQQ THE EMERGING MARKETS INTERNET ETF (EMQQ US) | 16.28 |
| 4 | ISHARES SMART CITY INFR USDA (CITY NA) | 15.95 |
| 5 | VANECK VIDEOGAMING ESPORTS (ESPO US) | 5.16 |

### สัดส่วนตามประเภทสินทรัพย์ (งวด 202607)

| รหัส | ประเภทสินทรัพย์ | มูลค่าตลาด | % NAV |
|---|---|---|---|
| 108 | หน่วยลงทุน (รหัส 108-109,117-121,139,130) | 74,298,057 | 98.8587 |
| 201 | เงินฝากธนาคาร P/N และ B/E (รหัส 202-205,216-224) | 1,352,234 | 1.7992 |
| 700 | สินทรัพย์อื่นๆ/หนี้สินอื่นๆ (รหัส 122,211,301-399,500-599) หัก (รหัส 601-699) | -494,477 | -0.6579 |

### รายการลงทุนรายตัวทั้งหมด (งวด 202606 · ณ 2026-06-30)

- จำนวนรายการที่ถือ: **13** รายการ จากผู้ออก 9 ราย
- มูลค่าทรัพย์สินสุทธิของกองทุน: **74,813,662** บาท
- น้ำหนัก 10 อันดับแรกรวม: **98.98% ของ NAV** (คิดเป็น 99.0% ของพอร์ต)

| # | หลักทรัพย์ | ผู้ออก | ประเภท | ISIN | มูลค่า | % NAV |
|---|---|---|---|---|---|---|
| 1 | [[../Entities/First Trust Portfolios\|First Trust Portfolios]] <br><sub>`US33734X8469`</sub> | FIRST TRUST PORTFOLIOS LP | หน่วยลงทุนของกองทุนตราสารทุน | US33734X8469 | 32,177,296 | 43.0099 |
| 2 | BlackRock Asset Management <br><sub>`IE00BKTLJC87NL`</sub> | BlackRock Asset Management | หน่วยลงทุนของกองทุนตราสารทุน | IE00BKTLJC87 | 12,382,335 | 16.5509 |
| 3 | GLOBAL X MANAGEMENT CO LLC <br><sub>`US37950E4162`</sub> | GLOBAL X MANAGEMENT CO LLC | หน่วยลงทุนของกองทุนตราสารทุน | US37950E4162 | 10,938,677 | 14.6212 |
| 4 | HAN ETF <br><sub>`US3015058890`</sub> | HAN ETF | หน่วยลงทุนของกองทุนตราสารทุน | US3015058890 | 8,350,808 | 11.1621 |
| 5 | [[../Entities/VanEck Digital Transformation ETF (US92189H8218)\|VanEck Digital Transformation ETF]] <br><sub>`US92189H8218`</sub> | Van Eck Global | หน่วยลงทุนของกองทุนตราสารทุน | US92189H8218 | 3,557,766 | 4.7555 |
| 6 | [[../Entities/State Street Financial Select Sector SPDR ETF (US81369Y6059)\|State Street Financial Select Sector SPDR ETF]] <br><sub>`US81369Y6059`</sub> | State Stree Bank and Turst | หน่วยลงทุนของกองทุนตราสารทุน | US81369Y6059 | 3,307,276 | 4.4207 |
| 7 | [[../Entities/VanEck Video Gaming and eSports ETF (isin-US92189F1140)\|VanEck Video Gaming and eSports ETF]] <br><sub>`US92189F1140`</sub> | Van Eck Global | หน่วยลงทุนของกองทุนตราสารทุน | US92189F1140 | 1,133,002 | 1.5144 |
| 8 | [[../Entities/Global X Video Games & Esports ETF (US37954Y3927)\|Global X Video Games & Esports ETF]] <br><sub>`US37954Y3927`</sub> | GLOBAL X MANAGEMENT CO LLC | หน่วยลงทุนของกองทุนตราสารทุน | US37954Y3927 | 981,483 | 1.3119 |
| 9 | [[../Entities/ETF MANAGERS GROUP\|ETF MANAGERS GROUP]] <br><sub>`US0321086565`</sub> | AMPLIFY INVESTMENTS | หน่วยลงทุนของกองทุนตราสารทุน | US0321086565 | 629,442 | 0.8414 |
| 10 | GLOBAL X MANAGEMENT CO LLC <br><sub>`US37954Y8140`</sub> | GLOBAL X MANAGEMENT CO LLC | หน่วยลงทุนของกองทุนตราสารทุน | US37954Y8140 | 593,024 | 0.7927 |
| 11 | [[../Entities/Bank Of Ayudhya Public Company Limited\|Bank Of Ayudhya Public Company Limited]] <br><sub>`CLBAY03`</sub> | BANK OF AYUDHYA PUBLIC COMPANY LIMITED | เงินฝากธนาคารประเภทออมทรัพย์ | - | 548,481 | 0.7331 |
| 12 | [[../Entities/JPMorgan Chased Bank, N.A\|JPMorgan Chased Bank, N.A.]] <br><sub>`CLJP01`</sub> | JPMorgan Chased Bank, N.A. | เงินฝากธนาคารประเภทออมทรัพย์ | - | 173,192 | 0.2315 |
| 13 | [[../Entities/Bank Of Ayudhya Public Company Limited\|Bank Of Ayudhya Public Company Limited]] <br><sub>`CABAY01`</sub> | BANK OF AYUDHYA PUBLIC COMPANY LIMITED | เงินฝากธนาคารประเภทกระแสรายวัน | - | 2, | 0.0027 |

### ข้อมูลเพิ่มเติมจาก Factsheet

ส่วนนี้ไม่มีใน API — แกะจากตารางใน Factsheet PDF

**การจัดสรรตามอันดับความน่าเชื่อถือ**

| อันดับ | % NAV |
|---|---|
| - | 3 |

> รายละเอียดทั้งหมดดูที่ [[Factsheet - BCAP-XDIGI]]

## 8. NAV

| Class | วันที่ | NAV/หน่วย | ราคาขาย | ราคารับซื้อคืน | มูลค่าทรัพย์สินสุทธิ |
|---|---|---|---|---|---|
| main | 2026-08-25 | 10.4828 | 10.595 | 10.4828 | 70,507,282 |

อ่านเพิ่ม: [[NAV และราคาซื้อขายหน่วยลงทุน]]

### NAV ย้อนหลัง 2024-04-04 → 2026-08-25 (ชนิด `main`)

`▄▄▄▄▃▂▄▇▇▆▆█▇▅▂▅▇▇▇█▇▇▇███▇▅▄▅▄▂▁▁▁▂▄▆▄▆▅▇`

| ช่วง | วันที่ | จำนวนวัน | เปลี่ยนแปลง | ความผันผวน (ต่อปี) |
|---|---|---|---|---|
| ทั้งช่วง | 2024-04-04 → 2026-08-25 | 500 | 5.97% | 22.7% |
| 1 ปี | 2025-08-25 → 2026-08-25 | 243 | -3.34% | 20.5% |

สูงสุด 11.4757 · ต่ำสุด 8.2572 (ตลอดช่วงที่เก็บ)

> ตัวเลข**คำนวณเองจาก NAV รายวัน** (ที่มา: `data/raw/nav.jsonl`) · เป็นข้อมูล**ย้อนหลัง** ไม่ใช่การพยากรณ์ · ผลตอบแทน/ผันผวนอดีตไม่รับประกันอนาคต · ช่วงที่กองอายุยังไม่ถึงจะไม่แสดง

## 9. เงินปันผล

| Class | นโยบายปันผล |
|---|---|
| main | ไม่จ่ายเงินปันผล |

## 10. การซื้อขายหน่วยลงทุน

### ยอดซื้อขายขั้นต่ำ

| Class | มีผลตั้งแต่ | ซื้อขั้นต่ำ (IPO) | สกุล (IPO) | ซื้อขั้นต่ำ | สกุล | ขายคืนขั้นต่ำ | สกุล (ขายคืน) | ยอดคงเหลือขั้นต่ำ | สกุล (คงเหลือ) |
|---|---|---|---|---|---|---|---|---|---|
| main | 2026-07-31 | 500.0 | THB | 500.0 | THB | - | THB | - | THB |

### ช่วงเวลาซื้อขายและการชำระเงิน

| Class | มีผลตั้งแต่ | ประเภทรายการ | ช่วงเวลาทำรายการ | ระยะเวลารับเงิน |
|---|---|---|---|---|
| main | 2026-07-31 | การซื้อ (subscription) | ทุกวันทำการ | - |
| main | 2026-07-31 | การขายคืน (redemption) | ทุกวันทำการ | T+3 คือ3 วันทำการหลังจากวันทำรายการขายคืน |

## 11. บุคคลที่เกี่ยวข้อง

| บทบาท | ชื่อ |
|---|---|
| นายทะเบียนหน่วยลงทุน | บริษัทหลักทรัพย์จัดการกองทุนรวม บัวหลวง จำกัด |
| T | บริษัทหลักทรัพย์จัดการกองทุนรวม บัวหลวง จำกัด |

## 12. Factsheet

📄 ข้อความที่แกะจาก PDF: [[Factsheet - BCAP-XDIGI]]

- class `main` (ณ 2026-07-31) — [PDF](https://secdocumentstorage.blob.core.windows.net/fundfactsheet/M0382_2566.pdf)

---

## ดูเพิ่ม

- กองอื่นของ [[บริษัทหลักทรัพย์จัดการกองทุนรวม บัวหลวง จำกัด]]
- กองประเภทเดียวกัน: [[ตราสารทุน]]
- ความเสี่ยงระดับเดียวกัน: [[by-risk|กองความเสี่ยงระดับ 6]]
- [[00-home|🏠 Home]] · [[all-funds|รายชื่อกองทุนทั้งหมด]]

> ข้อมูลจาก SEC Open API · อัปเดตล่าสุด `2026-08-27T07:42:31.783`
