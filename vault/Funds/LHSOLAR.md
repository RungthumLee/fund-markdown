---
title: "LHSOLAR"
proj_id: M0489_2569
regis_id: "MF0231_2569"
abbr: "LHSOLAR"
name_th: "กองทุนเปิด แอล เอช โซลาร์ เอเนอร์จี"
name_en: "LH SOLAR ENERGY FUND"
amc: "บริษัทหลักทรัพย์จัดการกองทุน แลนด์ แอนด์ เฮ้าส์ จำกัด"
amc_id: "C0000006646"
policy: "ตราสารทุน"
risk_spectrum: "6"
management_style: "PN"
retail_type: "R"
invest_country_flag: "1"
init_date: "2026-05-11"
regis_date: "2026-05-26"
class_count: 1
has_factsheet: true
ter_retail: 2.099
nav: 6.8719
nav_date: "2026-08-26"
fund_size: 459564531.22
holdings_count: 20
top10_pct_nav: 102.16
market_countries: ["สหรัฐฯ", "ฮ่องกง"]
country_top: "สหรัฐฯ"
country_coverage_pct: 45.1
master_fund: "Invesco Solar ETF"
tags: [fund, sec-data, asset/equity, risk/high, liquidity/t2, sector/technology, cap/large, fx/partially-hedged, struct/feeder, style/passive, use/accumulate]
---

# LHSOLAR

**กองทุนเปิด แอล เอช โซลาร์ เอเนอร์จี**  
_LH SOLAR ENERGY FUND_

บลจ. [[บริษัทหลักทรัพย์จัดการกองทุน แลนด์ แอนด์ เฮ้าส์ จำกัด]] · นโยบาย [[ตราสารทุน]] · ความเสี่ยงระดับ **6**

> [!abstract] สรุปย่อ
> - **รหัสโครงการ:** `M0489_2569`
> - **ประเภท:** ตราสารทุน · Feeder — กองหลัก passive
> - **ผู้ลงทุน:** ผู้ลงทุนทั่วไป
> - **ต่างประเทศ:** เน้นลงทุนต่างประเทศ
> - **จัดตั้ง:** 2026-05-11 · **จดทะเบียน:** 2026-05-26
> - **ชนิดหน่วยลงทุน:** 1 class
> - **NAV 6.8719 (2026-08-26)**

> [!tip] อ่านง่าย ๆ
> กองหุ้น · ความเสี่ยงสูง (6/8) · เหมาะกับสะสมระยะยาว · ขายคืนแล้วได้เงินภายใน T+2 · เน้นกลุ่มเทคโนโลยี · ลงทุนต่างประเทศ (สหรัฐฯ) · ป้องกันค่าเงินบางส่วน · ลงทุนผ่านกองทุนหลัก (feeder) · บริหารแบบอิงดัชนี (passive) · ค่าธรรมเนียมรวมที่รายย่อยจ่ายจริงราว 2.10%/ปี

## 1. ข้อมูลทั่วไป

| รายการ | ค่า |
|---|---|
| เลขที่โครงการ (proj_id) | `M0489_2569` |
| เลขที่จดทะเบียน (regis_id) | MF0231_2569 |
| ชื่อย่อ | LHSOLAR |
| ชื่อไทย | กองทุนเปิด แอล เอช โซลาร์ เอเนอร์จี |
| ชื่ออังกฤษ | LH SOLAR ENERGY FUND |
| บลจ. | [[บริษัทหลักทรัพย์จัดการกองทุน แลนด์ แอนด์ เฮ้าส์ จำกัด]] |
| สถานะ | Registered |
| วันจัดตั้ง | 2026-05-11 |
| วันจดทะเบียน | 2026-05-26 |
| ประเภทตามนโยบาย | ตราสารทุน |
| กลยุทธ์บริหาร | PN — Feeder — กองหลัก passive |
| ลักษณะโครงการ | R — ผู้ลงทุนทั่วไป |
| ความเสี่ยงต่างประเทศ | เน้นลงทุนต่างประเทศ |

### กองทุนหลัก (Feeder Fund)

- **กองทุนหลัก:** Invesco Solar ETF
- **ประเทศ:** สหรัฐอเมริกา
- 🌐 **โน้ตกองทุนหลัก:** [[Invesco Solar ETF]] — ค่าธรรมเนียม ขนาดกองทุน sector และหลักทรัพย์ที่ถือจริง

ดูแนวคิดที่ [[Feeder Fund]]

### นโยบายป้องกันความเสี่ยงอัตราแลกเปลี่ยน

ดุลยพินิจ (dynamic hedging) (0%-105% ของมูลค่าความเสี่ยง)

## 2. ชนิดหน่วยลงทุน (Share Class)

| Class | ISIN | สิทธิประโยชน์ภาษี | รายละเอียด |
|---|---|---|---|
| main | - | - | - |

## 3. นโยบายการลงทุน

กองทุนมีนโยบายลงทุนในหน่วยลงทุนของกองทุนรวมต่างประเทศ (หรือ “กองทุนหลัก (Master fund)”) ที่มีนโยบายลงทุนในตราสารทุน โดยจะลงทุนในหน่วยลงทุนของกองทุนหลักโดยเฉลี่ยในรอบปีบัญชีไม่น้อยกว่าร้อยละ 80 ของมูลค่าทรัพย์สินสุทธิของกองทุนซึ่งกองทุนไทยจะลงทุนในหน่วยลงทุนของกองทุน Invesco Solar ETF (“กองทุนหลัก”) สกุลเงินดอลลาร์สหรัฐฯ (USD) ที่จดทะเบียนซื้อขายในตลาดหลักทรัพย์นิวยอร์ก (NYSE Arca) เพียงตลาดหลักทรัพย์เดียว ซึ่งกองทุนหลักได้จดทะเบียนจัดตั้งในประเทศสหรัฐอเมริกา (USA) โดยกองทุนบริหารจัดการลงทุน และมีที่ปรึกษาการลงทุนคือ Invesco Capital Management LLC (the “Adviser”)
โดยกองทุน Invesco Solar ETF มีวัตถุประสงค์หรือกลยุทธ์ในการลงทุนเพื่อสร้างผลตอบแทนให้ใกล้เคียงกับดัชนี MAC Global Solar Energy Index (“ดัชนีอ้างอิง”) ซึ่งเป็นดัชนีที่ถูกออกแบบมาเพื่อให้สะท้อนการลงทุนในบริษัทที่ประกอบธุรกิจเกี่ยวข้องกับพลังงานแสงอาทิตย์ในระดับโลก (global solar energy businesses) โดยดัชนีอ้างอิงประกอบด้วยบริษัทที่จดทะเบียนซื้อขายในตลาดหลักทรัพย์ของประเทศที่กำหนด และมีรายได้ไม่น้อยกว่าหนึ่งในสามของรายได้รวมจากกิจกรรมที่เกี่ยวข้องกับพลังงานแสงอาทิตย์เพื่อผลิตพลังงานความร้อนหรือพลังงานไฟฟ้า รวมถึงบริษัทที่จำหน่ายไฟฟ้าที่ผลิตจากพลังงานแสงอาทิตย์ โดยกองทุนหลักมุ่งสร้างผลตอบแทนของกองทุนก่อนหักค่าใช้จ่ายหรือค่าธรรมเนียมต่าง ๆ ของกองทุนใกล้เคียงกับผลตอบแทนของดัชนี MAC Global Solar Energy Index มากที่สุด ทั้งนี้ กองทุนจะลงทุนในต่างประเทศซึ่งจะส่งผลให้มี net exposure ที่เกี่ยวข้องกับความเสี่ยงต่างประเทศโดยเฉลี่ยในรอบปีบัญชีไม่น้อยกว่าร้อยละ 80 ของมูลค่าทรัพย์สินสุทธิของกองทุน หรือตามอัตราส่วนที่สำนักงานคณะกรรมการ ก.ล.ต. ประกาศกำหนด
สำหรับเงินลงทุนส่วนที่เหลือกองทุนไทยอาจพิจารณาลงทุนในหน่วยลงทุนของกองทุนรวมอีทีเอฟที่มีการบริหารจัดการกองทุนในลักษณะ inverse management เพื่อบริหารจัดการพอร์ตการลงทุนของกองทุนรวมเท่านั้น (portfolio management) และจะพิจารณาลงทุนทั้งในและต่างประเทศ ในตราสารหนี้ที่มีลักษณะคล้ายเงินฝาก เงินฝาก ตราสารหนี้ ตราสารทุน ตราสารกึ่งหนี้กึ่งทุน ใบสำคัญแสดงสิทธิ รวมถึงหลักทรัพย์ และ/หรือทรัพย์สินอื่นหรือการหาดอกผลโดยวิธีอื่นตามที่ระบุไว้ในโครงการ และ/หรือตามที่คณะกรรมการ ก.ล.ต. หรือ สำนักงานคณะกรรมการ ก.ล.ต. ประกาศกำหนดหรือให้ความเห็นชอบ
กองทุนอาจลงทุนในสัญญาซื้อขายล่วงหน้า (Derivatives) เพื่อป้องกันความเสี่ยง (Hedging) จากอัตราแลกเปลี่ยน (Foreign Exchange Rate Risk) ตามดุลยพินิจของผู้จัดการกองทุน (ระหว่างร้อยละ 0 – 105 ของมูลค่าเงินลงทุนในต่างประเทศ) โดยในการลงทุนในสัญญาซื้อขายล่วงหน้าเพื่อป้องกันความเสี่ยงด้านอัตราแลกเปลี่ยน ผู้จัดการกองทุนจะพิจารณาจากสภาวะของตลาดการเงินในขณะนั้น และปัจจัยอื่นที่เกี่ยวข้อง เช่น ปัจจัยทางเศรษฐกิจ การเมือง การเงินและการคลัง เป็นต้น เพื่อคาดการณ์ทิศทางแนวโน้มของค่าเงิน ในกรณีที่คาดการณ์ว่าค่าเงินดอลลาร์สหรัฐ (USD) มีแนวโน้มอ่อนค่าลง ผู้จัดการกองทุนอาจพิจารณาลงทุนในสัญญาซื้อขายล่วงหน้าเพื่อป้องกันความเสี่ยงด้านอัตราแลกเปลี่ยนเงินตราต่างประเทศในสัดส่วนที่ค่อนข้างมาก แต่หากในกรณีที่ค่าเงินดอลลาร์สหรัฐ (USD) มีแนวโน้มแข็งค่าขึ้น ผู้จัดการกองทุนอาจพิจารณาไม่ลงทุนในสัญญาซื้อขายล่วงหน้าเพื่อป้องกันความเสี่ยงด้านอัตราแลกเปลี่ยนเงินตราต่างประเทศ หรืออาจลงทุนในสัดส่วนน้อย อย่างไรก็ตามหากผู้จัดการกองทุนพิจารณาว่าการป้องกันความเสี่ยงอัตราแลกเปลี่ยนอาจไม่เป็นไปเพื่อประโยชน์ของผู้ถือหน่วยลงทุน หรือทำให้ผู้ถือหน่วยลงทุนเสียประโยชน์ที่อาจได้รับ ผู้จัดการกองทุนอาจไม่ป้องกันความเสี่ยงจากอัตราแลกเปลี่ยนก็ได้ นอกจากนี้การทำธุรกรรมป้องกันความเสี่ยงดังกล่าวอาจมีต้นทุนซึ่งทำให้ผลตอบแทนของกองทุนโดยรวมลดลงจากต้นทุนที่เพิ่มขึ้น
นอกจากนี้ กองทุนอาจลงทุนในสัญญาซื้อขายล่วงหน้า (Derivatives) เพื่อลดความเสี่ยง (Hedging) ที่มี underlying เป็นอัตราแลกเปลี่ยน เพื่อให้ลดความผันผวนของกองทุน และ/หรือลดค่าใช้จ่ายของกองทุน หรือตามที่สำนักงานคณะกรรมการ ก.ล.ต. ประกาศกำหนด
สัญญาซื้อขายล่วงหน้าที่กองทุนจะเข้าเป็นคู่สัญญาจะมีสินทรัพย์อ้างอิง ได้แก่ อัตราแลกเปลี่ยนเงิน หากเกิดเหตุการณ์ที่ส่งผลกระทบต่อสัญญาหรือคู่สัญญาที่ทำธุรกรรมไม่สามารถชำระภาระผูกพันได้ตามกำหนดเวลา อาจทำให้กองทุนไม่ได้รับผลตอบแทนตามที่คาดหวัง หรือขาดทุนจากการลงทุนในสัญญาซื้อขายล่วงหน้าได้ อย่างไรก็ตาม บริษัทจัดการจะวิเคราะห์และติดตามปัจจัยที่มีผลกระทบต่อราคาสินทรัพย์อ้างอิง รวมถึงพิจารณาความน่าเชื่อถือของคู่สัญญาก่อนทำธุรกรรมและติดตามอย่างต่อเนื่อง โดยกองทุนจะเน้นทำธุรกรรมกับคู่สัญญาที่มีอันดับ …(ตัดทอน)

### ลักษณะเฉพาะของโครงการ

- `CIV` กองทุนรวมที่มีนโยบายเปิดให้มีการลงทุนในกองทุนรวมอื่นภายใต้ บลจ. เดียวกัน (CROSS Investing Fund)
- `FED` กองทุนรวมฟีดเดอร์ (Feeder Fund )

## 4. ความเสี่ยง

**ระดับความเสี่ยง: 6 / 8** — 6 — ตราสารทุน

> ลงทุนในตราสารทุนโดยมี net exposure โดยเฉลี่ยในรอบปีบัญชีไม่น้อยกว่า 80 % ของ NAV

อ่านเพิ่ม: [[ระดับความเสี่ยงกองทุนรวม]]

### สถิติเชิงปริมาณ

| Class | Sharpe | Alpha | Beta | Max Drawdown | Tracking Error | Turnover | FX Hedging |
|---|---|---|---|---|---|---|---|
| main | -1.7815 | -0.0517 | 0.8612 | -32.32% | - | 1.4849 | 87.635338624027 |

อ่านเพิ่ม: [[สถิติวัดผลกองทุน]]

## 5. ค่าธรรมเนียม

ดูคำอธิบายแต่ละประเภทที่ [[ค่าธรรมเนียมกองทุนรวม]]

### สรุปต่อชนิดหน่วยลงทุน (Share Class)

| ชนิดหน่วยลงทุน | ซื้อได้โดย | รวม: เก็บจริง (%) | รวม: เพดาน (%) | จัดการ: เก็บจริง (%) | ขาย (%) | รับซื้อคืน (%) |
|---|---|---|---|---|---|---|
| `main` | ⚪ ไม่ระบุ | 2.099 | 4.28 | 1.6 | 1.605 | - |

**เก็บจริง** = อัตราที่เรียกเก็บในรอบที่รายงาน · **เพดาน** = อัตราสูงสุดที่หนังสือชี้ชวนอนุญาต — สองค่านี้เทียบกันข้ามกองไม่ได้

อธิบายชนิดหน่วยลงทุนที่ [[ชนิดหน่วยลงทุน Share Class]] · [[../Indexes/compare-fees|เทียบกับกองอื่นในหมวดเดียวกัน]]

### ตาม Factsheet ล่าสุด (ทุกแถวที่รายงาน)

| Class | ประเภท | เพดาน (%) | เก็บจริง (%) |
|---|---|---|---|
| main | ค่าธรรมเนียมการขายหน่วยลงทุน (Front-end Fee) | 2.5 | 1.605 |
| main | ค่าธรรมเนียมการรับซื้อคืนหน่วยลงทุน (Back-end Fee) | 2.5 | 0 |
| main | ค่าธรรมเนียมการสับเปลี่ยนหน่วยลงทุนเข้า (SWITCHING IN) | 2.5 | 1.605 |
| main | ค่าธรรมเนียมการจัดการ (Management Fee) | 2.5 | 1.6 |
| main | ค่าธรรมเนียมการสับเปลี่ยนหน่วยลงทุนออก (SWITCHING OUT) | 2.5 | 0 |
| main | ค่าธรรมเนียมและค่าใช้จ่ายรวมทั้งหมด (Total Fee and Expense) | 4.28 | 2.099 |

### ตามหนังสือชี้ชวน (เพดานตามโครงการ)

| Class | ประเภท | อัตรา | หน่วย |
|---|---|---|---|
| main | ค่าธรรมเนียมการจัดการ (Management Fee) | 2.5 | ของมูลค่าทรัพย์สินสุทธิของกองทุนรวมที่คำนวณได้เป็นรายวัน |
| main | ค่าธรรมเนียมทั้งหมด (Total Fee) | 5 | ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมที่ปรึกษาการลงทุน (Investment Advisor Fee) | 0.5 | ของมูลค่าทรัพย์สินสุทธิของกองทุนที่คำนวณได้เป็นรายวัน |
| main | ค่าธรรมเนียมนายทะเบียนหน่วยลงทุน (Registrar Fee) | 2.5 | ของมูลค่าทรัพย์สินสุทธิของกองทุนรวมที่คำนวณได้เป็นรายวัน |
| main | ค่าธรรมเนียมผู้จัดจำหน่ายหน่วยลงทุน (Distributor Fee) | - | - |
| main | ค่าธรรมเนียมผู้ดูแลผลประโยชน์ (Trustee Fee) | 2.5 | ของมูลค่าทรัพย์สินสุทธิของกองทุนรวมที่คำนวณได้เป็นรายวัน |
| main | ค่าธรรมเนียมอื่น ๆ (Other Fee) | 1.4 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวมที่คำนวณได้เป็นรายวัน |

## 6. ผลการดำเนินงาน

_ไม่มีข้อมูลผลการดำเนินงานจาก API_

## 7. พอร์ตการลงทุน

### การกระจายตามประเทศ (ตลาดจริงจากหลักทรัพย์)

| ประเทศ (ตลาดที่ซื้อขาย) | % ของกอง |
|---|---|
| สหรัฐฯ | 41.7 |
| ฮ่องกง | 3.4 |

> ทะลุถึงหลักทรัพย์จริงได้ **45.1%** ของกอง (ส่วนที่เหลือ 54.9% ยังไม่ทะลุ) · ที่มา: look-through (Yahoo top holdings) · symbol/ISIN

### ⚖️ ปัจจัยที่กระทบกอง (สองด้าน)

> [!INFO] ปัจจัยที่กองนี้**ไว**ต่อ — อ่านจากหมวด/ประเทศ/โครงสร้างที่ถือจริง
> แสดง**ทั้งสองด้าน** (ขึ้น=โอกาส · ลง=ความเสี่ยง) · **ไม่ใช่การพยากรณ์** ว่าจะไปทางไหน · [กรอบข้อมูลอ้างอิง](../../docs/project/ideas.md)

| ปัจจัย | หมวด | ถ้าขึ้น ▲ | ถ้าลง ▼ | จาก |
|---|---|---|---|---|
| ค่าธรรมเนียมรวมสูง (2.10%/ปี) | โครงสร้าง | - | ค่าธรรมเนียมยิ่งสูง ยิ่งกินผลตอบแทนทุกปีไม่ว่าตลาดขึ้นหรือลง | โครงสร้างกอง |
| นโยบายรัฐบาลจีน / ภาคอสังหา | Macro | มาตรการกระตุ้น → ตลาดฟื้น | คุมเข้ม/วิกฤตอสังหา → กดตลาด | ประเทศ: ฮ่องกง |
| วัฏจักรเซมิคอนดักเตอร์ / การลงทุน AI | Sector | วัฏจักรชิปขาขึ้น / AI capex โต → รายได้เพิ่ม | ชิปล้นตลาด / capex ชะลอ → รายได้หด | หมวด: technology |
| อัตราดอกเบี้ย (กดหุ้นเติบโต) | Macro | ดอกเบี้ยลง → หุ้นเติบโต valuation สูงขึ้น | ดอกเบี้ยขึ้น → กด valuation หุ้นเทค | หมวด: technology |
| ความตึงเครียดจีน-สหรัฐ / การกีดกันเทคโนโลยี | Macro | ผ่อนคลายข้อจำกัด → ห่วงโซ่อุปทานคล่อง | คว่ำบาตร/กีดกันชิป → กระทบรายได้/ซัพพลายเชน | หมวด: technology |
| ดอกเบี้ยเฟด / เศรษฐกิจสหรัฐ | Macro | เศรษฐกิจแข็งแรง/เฟดผ่อนคลาย | เฟดขึ้นดอกเบี้ย/เศรษฐกิจชะลอ | ประเทศ: สหรัฐฯ |


### 📊 เคลื่อนไหวสัมพันธ์กับอะไร (correlation อดีต)

| ปัจจัย | correlation | ทิศทาง | จำนวนวัน |
|---|---|---|---|
| หุ้นสหรัฐ (S&P 500) | +0.71 | ไปทางเดียวกัน | 60 |
| หุ้นตลาดเกิดใหม่ (EM) | +0.69 | ไปทางเดียวกัน | 60 |

> **วัดจาก NAV รายวัน** ช่วง 2026-05-25..2026-08-26 (ที่มา: NAV × Yahoo) · +1 = ไปทางเดียวกัน · −1 = สวนทาง
> [!warning] เป็น**ความสัมพันธ์ที่วัดจากอดีตในช่วงข้างต้น** — **ไม่ใช่การพยากรณ์** · correlation ไม่ใช่สาเหตุ และ**ไม่นิ่ง** (มักพุ่งเข้า 1 ตอนตลาดวิกฤต)
### การจัดสรรสินทรัพย์ (จาก Factsheet)

| สินทรัพย์ | สัดส่วน (%) |
|---|---|
| หน่วยลงทุน Invesco Solar ETF | 99.2552 |
| เงินฝากธนาคาร | 7.361 |

### สัดส่วนตามประเภทสินทรัพย์ (งวด 202607)

| รหัส | ประเภทสินทรัพย์ | มูลค่าตลาด | % NAV |
|---|---|---|---|
| 108 | หน่วยลงทุน (รหัส 108-109,117-121,139,130) | 394,285,005 | 99.2552 |
| 201 | เงินฝากธนาคาร P/N และ B/E (รหัส 202-205,216-224) | 29,463,867 | 7.4171 |
| 401 | ตราสารอนุพันธ์(รหัส 401-407) | -9,793,112 | -2.4653 |
| 700 | สินทรัพย์อื่นๆ/หนี้สินอื่นๆ (รหัส 122,211,301-399,500-599) หัก (รหัส 601-699) | -16,712,237 | -4.207 |

### รายการลงทุนรายตัวทั้งหมด (งวด 202606 · ณ 2026-06-30)

- จำนวนรายการที่ถือ: **20** รายการ จากผู้ออก 2 ราย
- มูลค่าทรัพย์สินสุทธิของกองทุน: **447,691,35** บาท
- น้ำหนัก 10 อันดับแรกรวม: **102.16% ของ NAV** (คิดเป็น 101.6% ของพอร์ต)

> [!NOTE]
> น้ำหนักรวมเกิน 100% ของ NAV ได้ตามปกติ เพราะตารางนี้แสดง**ฐานะขั้นต้น** (หน่วยลงทุนกองหลัก + เงินฝาก) 
> ซึ่งถูกหักกลบด้วยรายการหนี้สินที่มีค่าติดลบในตารางเดียวกัน ไม่ใช่การใช้ leverage

| # | หลักทรัพย์ | ผู้ออก | ประเภท | ISIN | มูลค่า | % NAV |
|---|---|---|---|---|---|---|
| 1 | INVESCO SOLAR ETF <br><sub>`TAN US`</sub> | Invesco Solar ETF | หน่วยลงทุนของกองทุนแบบอื่นๆ | - | 443,263,389 | 99.0109 |
| 2 | [[../Entities/Krung Thai Bank Pcl\|Krung Thai Bank Pcl.]] <br><sub>`0070340021`</sub> | KRUNG THAI BANK PUBLIC COMPANY LIMITED | เงินฝากธนาคารประเภทออมทรัพย์ | - | 14,398,465 | 3.2162 |
| 3 | [[../Entities/Krung Thai Bank Pcl\|Krung Thai Bank Pcl.]] <br><sub>`SA-USD-LHSOLAR`</sub> | KRUNG THAI BANK PUBLIC COMPANY LIMITED | เงินฝากธนาคารประเภทออมทรัพย์ | - | 276,176 | 0.0617 |
| 4 | FX-TX.1998885 | - | สัญญาฟอร์เวิร์ด | - | 256,79 | 0.0574 |
| 5 | FX-TX.2001856 | - | สัญญาฟอร์เวิร์ด | - | 161,561 | 0.0361 |
| 6 | FX-TX.2011959 | - | สัญญาฟอร์เวิร์ด | - | 62,92 | 0.014 |
| 7 | FX-TX.1989685 | - | สัญญาฟอร์เวิร์ด | - | -79,699 | -0.0178 |
| 8 | FX-TX.2011020 | - | สัญญาฟอร์เวิร์ด | - | -158,886 | -0.0355 |
| 9 | FX-TX.2000036 | - | สัญญาฟอร์เวิร์ด | - | -374,289 | -0.0836 |
| 10 | FX-TX.1997833 | - | สัญญาฟอร์เวิร์ด | - | -435,56 | -0.0973 |
| 11 | FX-TX.1995312 | - | สัญญาฟอร์เวิร์ด | - | -609,782 | -0.1362 |
| 12 | FX-TX.1996442 | - | สัญญาฟอร์เวิร์ด | - | -626,146 | -0.1399 |
| 13 | FX-TX.2005829 | - | สัญญาฟอร์เวิร์ด | - | -659,616 | -0.1473 |
| 14 | FX-TX.2003700 | - | สัญญาฟอร์เวิร์ด | - | -669,206 | -0.1495 |
| 15 | FX-TX.2003702 | - | สัญญาฟอร์เวิร์ด | - | -678,837 | -0.1516 |
| 16 | FX-TX.1995315 | - | สัญญาฟอร์เวิร์ด | - | -762,485 | -0.1703 |
| 17 | FX-TX.1995314 | - | สัญญาฟอร์เวิร์ด | - | -770,506 | -0.1721 |
| 18 | FX-TX.1995313 | - | สัญญาฟอร์เวิร์ด | - | -775,598 | -0.1732 |
| 19 | FX-TX.1996439 | - | สัญญาฟอร์เวิร์ด | - | -801,188 | -0.179 |
| 20 | FX-TX.2004626 | - | สัญญาฟอร์เวิร์ด | - | -897,243 | -0.2004 |

### 🔭 ทะลุกองทุนหลัก (Look-through)

กองนี้ถือ **Invesco Solar ETF** อยู่ **99.01%** ของ NAV ตารางด้านล่างคูณต่อด้วยสัดส่วนที่กองทุนหลักถือหลักทรัพย์แต่ละตัว

> [!CAUTION] ตัวเลขนี้เป็น **ขั้นต่ำ** ไม่ใช่สัดส่วนที่แท้จริง
> ใช้เฉพาะหลักทรัพย์ **10 อันดับแรก** ที่กองทุนหลักเปิดเผย รวมกันได้เพียง **57.15%** จาก 99.01% ที่กองถืออยู่
> ส่วนที่เหลืออยู่ในหลักทรัพย์ที่ไม่ได้เปิดเผยรายตัว และวันอ้างอิงของสองฝั่งไม่ตรงกัน
> อ่าน [[Look-through การถือทางอ้อม|ข้อจำกัดฉบับเต็ม]] ก่อนใช้ตัดสินใจ

| # | หลักทรัพย์ | Symbol | % ของกองทุนหลัก | ~% ของกองนี้ |
|---|---|---|---|---|
| 1 | [[../Entities/First Solar Inc\|First Solar Inc]] | FSLR | 9.96 | 9.8615 |
| 2 | [[../Entities/Nextpower Inc Class A\|Nextpower Inc Class A]] | NXT | 9.31 | 9.2179 |
| 3 | Enlight Renewable Energy Ltd | ENLT.TA | 7.57 | 7.4951 |
| 4 | [[../Entities/Enphase Energy Inc\|Enphase Energy Inc]] | ENPH | 6.51 | 6.4456 |
| 5 | Doral Group Renewable Eneryy Resources Ltd Ordinary Shares | DORL.TA | 4.65 | 4.604 |
| 6 | SolarEdge Technologies Inc | SEDG | 4.32 | 4.2773 |
| 7 | [[../Entities/HA Sustainable Infrastructure Capital Inc\|HA Sustainable Infrastructure Capital Inc]] | HASI | 4.27 | 4.2278 |
| 8 | [[../Entities/Sunrun Inc\|Sunrun Inc]] | RUN | 4.19 | 4.1486 |
| 9 | Clearway Energy Inc Class C | CWEN | 3.53 | 3.4951 |
| 10 | GCL Technology Holdings Ltd | 3800.HK | 3.41 | 3.3763 |

[[../Indexes/by-lookthrough|ดัชนีการถือทางอ้อมทั้งหมด]] · [[ค่าธรรมเนียมสองชั้นของ Feeder Fund]]

## 8. NAV

| Class | วันที่ | NAV/หน่วย | ราคาขาย | ราคารับซื้อคืน | มูลค่าทรัพย์สินสุทธิ |
|---|---|---|---|---|---|
| main | 2026-08-26 | 6.8719 | 6.9823 | 6.8719 | 459,564,531 |

อ่านเพิ่ม: [[NAV และราคาซื้อขายหน่วยลงทุน]]

### NAV ย้อนหลัง 2026-05-25 → 2026-08-26 (ชนิด `main`)

`██████▇▆▅▅▄▅▅▅▄▄▄▄▅▄▄▄▃▃▄▄▃▃▃▃▂▃▃▂▃▃▂▂▂▂▂▂▁▂▁▁▁▂▁▁▂▂▂▂▂▁▁▁▁▁▁▁▁`

| ช่วง | วันที่ | จำนวนวัน | เปลี่ยนแปลง | ความผันผวน (ต่อปี) |
|---|---|---|---|---|
| ทั้งช่วง | 2026-05-25 → 2026-08-26 | 63 | -31.28% | 39.6% |

สูงสุด 10.4114 · ต่ำสุด 6.8095 (ตลอดช่วงที่เก็บ)

> ตัวเลข**คำนวณเองจาก NAV รายวัน** (ที่มา: `data/raw/nav.jsonl`) · เป็นข้อมูล**ย้อนหลัง** ไม่ใช่การพยากรณ์ · ผลตอบแทน/ผันผวนอดีตไม่รับประกันอนาคต · ช่วงที่กองอายุยังไม่ถึงจะไม่แสดง

## 9. เงินปันผล

| Class | นโยบายปันผล |
|---|---|
| main | ไม่จ่ายเงินปันผล |

## 10. การซื้อขายหน่วยลงทุน

### ยอดซื้อขายขั้นต่ำ

| Class | มีผลตั้งแต่ | ซื้อขั้นต่ำ (IPO) | สกุล (IPO) | ซื้อขั้นต่ำ | สกุล | ขายคืนขั้นต่ำ | สกุล (ขายคืน) | ยอดคงเหลือขั้นต่ำ | สกุล (คงเหลือ) |
|---|---|---|---|---|---|---|---|---|---|
| main | 2026-07-31 | 1000.0 | THB | 100.0 | THB | - | THB | 100.0 | THB |

### ช่วงเวลาซื้อขายและการชำระเงิน

| Class | มีผลตั้งแต่ | ประเภทรายการ | ช่วงเวลาทำรายการ | ระยะเวลารับเงิน |
|---|---|---|---|---|
| main | 2026-07-31 | การซื้อ (subscription) | ทุกวันทำการ | - |
| main | 2026-07-31 | การขายคืน (redemption) | ทุกวันทำการ | T+2 คือ2 วันทำการหลังจากวันทำรายการขายคืน |

## 11. บุคคลที่เกี่ยวข้อง

| บทบาท | ชื่อ |
|---|---|
| นายทะเบียนหน่วยลงทุน | บริษัทหลักทรัพย์จัดการกองทุน แลนด์ แอนด์ เฮ้าส์ จำกัด |
| T | บริษัทหลักทรัพย์จัดการกองทุน แลนด์ แอนด์ เฮ้าส์ จำกัด |

## 12. Factsheet

📄 ข้อความที่แกะจาก PDF: [[Factsheet - LHSOLAR]]

- class `main` (ณ 2026-07-31) — [PDF](https://secdocumentstorage.blob.core.windows.net/fundfactsheet/M0489_2569.pdf)

---

## ดูเพิ่ม

- กองอื่นของ [[บริษัทหลักทรัพย์จัดการกองทุน แลนด์ แอนด์ เฮ้าส์ จำกัด]]
- กองประเภทเดียวกัน: [[ตราสารทุน]]
- ความเสี่ยงระดับเดียวกัน: [[by-risk|กองความเสี่ยงระดับ 6]]
- กองทุนหลักที่ลงทุนจริง: [[Invesco Solar ETF]]
- [[00-home|🏠 Home]] · [[all-funds|รายชื่อกองทุนทั้งหมด]]

> ข้อมูลจาก SEC Open API · อัปเดตล่าสุด `2026-08-27T07:42:31.783`
