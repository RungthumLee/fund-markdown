---
title: "ASP-MEM"
proj_id: M0696_2569
regis_id: "MF0315_2569"
abbr: "ASP-MEM"
name_th: "กองทุนเปิด แอสเซทพลัส เมมโมรี่ ชิป อิควิตี้ ห้ามขายผู้ลงทุนรายย่อย"
name_en: "Asset Plus Memory Chip Equity Fund Not for Retail Investors"
amc: "บริษัทหลักทรัพย์จัดการกองทุน แอสเซท พลัส จำกัด"
amc_id: "C0000005022"
policy: "ตราสารทุน"
risk_spectrum: "7"
management_style: "AN"
retail_type: "A"
invest_country_flag: "1"
init_date: "2026-07-01"
regis_date: "2026-07-09"
class_count: 1
has_factsheet: true
ter_retail: 2.64
nav: 8.8227
nav_date: "2026-08-26"
fund_size: 117890225.0
master_fund: "Roundhill Memory ETF"
fund_managers: ["นาย กมลยศ สุขุมสุวรรณ", "น.ส. ภัทรลภา รัตนดิลก ณ ภูเก็ต"]
tags: [fund, sec-data, asset/equity, risk/very-high, liquidity/t5, fx/discretionary, struct/feeder, style/active, use/accumulate, audience/restricted]
---

# ASP-MEM

**กองทุนเปิด แอสเซทพลัส เมมโมรี่ ชิป อิควิตี้ ห้ามขายผู้ลงทุนรายย่อย**  
_Asset Plus Memory Chip Equity Fund Not for Retail Investors_

บลจ. [[บริษัทหลักทรัพย์จัดการกองทุน แอสเซท พลัส จำกัด]] · นโยบาย [[ตราสารทุน]] · ความเสี่ยงระดับ **7**

> [!abstract] สรุปย่อ
> - **รหัสโครงการ:** `M0696_2569`
> - **ประเภท:** ตราสารทุน · Feeder — กองหลัก active
> - **ผู้ลงทุน:** ผู้ลงทุนที่มิใช่รายย่อย (AI)
> - **ต่างประเทศ:** เน้นลงทุนต่างประเทศ
> - **จัดตั้ง:** 2026-07-01 · **จดทะเบียน:** 2026-07-09
> - **ชนิดหน่วยลงทุน:** 1 class
> - **NAV 8.8227 (2026-08-26)**

> [!tip] อ่านง่าย ๆ
> กองหุ้น · ความเสี่ยงสูงมาก (7/8) · เหมาะกับสะสมระยะยาว · ขายคืนแล้วได้เงินภายใน T+5 · ลงทุนต่างประเทศ · ป้องกันค่าเงินตามดุลยพินิจผู้จัดการ · ลงทุนผ่านกองทุนหลัก (feeder) · บริหารเชิงรุก (active) · ค่าธรรมเนียมรวมที่รายย่อยจ่ายจริงราว 2.64%/ปี

## 1. ข้อมูลทั่วไป

| รายการ | ค่า |
|---|---|
| เลขที่โครงการ (proj_id) | `M0696_2569` |
| เลขที่จดทะเบียน (regis_id) | MF0315_2569 |
| ชื่อย่อ | ASP-MEM |
| ชื่อไทย | กองทุนเปิด แอสเซทพลัส เมมโมรี่ ชิป อิควิตี้ ห้ามขายผู้ลงทุนรายย่อย |
| ชื่ออังกฤษ | Asset Plus Memory Chip Equity Fund Not for Retail Investors |
| บลจ. | [[บริษัทหลักทรัพย์จัดการกองทุน แอสเซท พลัส จำกัด]] |
| สถานะ | Registered |
| วันจัดตั้ง | 2026-07-01 |
| วันจดทะเบียน | 2026-07-09 |
| ประเภทตามนโยบาย | ตราสารทุน |
| กลยุทธ์บริหาร | AN — Feeder — กองหลัก active |
| ลักษณะโครงการ | A — ผู้ลงทุนที่มิใช่รายย่อย (AI) |
| ความเสี่ยงต่างประเทศ | เน้นลงทุนต่างประเทศ |

### กองทุนหลัก (Feeder Fund)

- **กองทุนหลัก:** Roundhill Memory ETF
- **ประเทศ:** สหรัฐอเมริกา
- 🌐 **โน้ตกองทุนหลัก:** [[Roundhill Memory ETF]] — ค่าธรรมเนียม ขนาดกองทุน sector และหลักทรัพย์ที่ถือจริง

ดูแนวคิดที่ [[Feeder Fund]]

### นโยบายป้องกันความเสี่ยงอัตราแลกเปลี่ยน

ดุลยพินิจ (dynamic hedging) (0%-105% ของมูลค่าความเสี่ยง)

## 2. ชนิดหน่วยลงทุน (Share Class)

| Class | ISIN | สิทธิประโยชน์ภาษี | รายละเอียด |
|---|---|---|---|
| main | - | - | - |

## 3. นโยบายการลงทุน

กองทุนมีนโยบายเน้นลงทุนในกองทุน Roundhill Memory ETF (กองทุนหลัก) ซึ่งเป็นกองทุนรวมอีทีเอฟ (Exchange Traded Fund) ที่จดทะเบียนซื้อขายในตลาดหลักทรัพย์ Cboe BZX Exchange, Inc. (Cboe BZX) ประเทศสหรัฐอเมริกา (United States) เพียงตลาดหลักทรัพย์แห่งเดียว และกองทุนหลักจดทะเบียนจัดตั้งในประเทศสหรัฐอเมริกา (United States) โดยมี Roundhill Financial Inc. ทำหน้าที่เป็นที่ปรึกษาการลงทุน (Investment Adviser) และมี Exchange Traded Concepts, LLC (ETC) ทำหน้าที่เป็นที่ปรึกษาการลงทุนย่อย (Sub-Adviser) ซึ่งรับผิดชอบในการบริหารการลงทุนของกองทุน และอยู่ภายใต้การกำกับดูแลของ U.S. Securities and Exchange Commission (SEC) ซึ่งเป็นสมาชิกสามัญของ International Organization of Securities Commissions (IOSCO) ทั้งนี้ จะมี Net Exposure ในกองทุนหลักดังกล่าวโดยเฉลี่ยในรอบปีบัญชีไม่น้อยกว่าร้อยละ 80 ของมูลค่าทรัพย์สินสุทธิของกองทุน
กองทุนหลักมีวัตถุประสงค์มุ่งสร้างการเพิ่มค่าของเงินลงทุน (capital appreciation) จากการลงทุนในตราสารทุนของบริษัทที่ประกอบธุรกิจเกี่ยวกับหน่วยความจำ (Memory Companies) และอาจลงทุนในตราสารอนุพันธ์ เช่น Swap Agreements และ Forward Contracts เพื่อให้มีฐานะการลงทุนหรือความเสี่ยงที่อ้างอิงกับ Memory Companies ทั้งนี้ กองทุนหลักคัดเลือกหลักทรัพย์ด้วยวิธีการที่พัฒนาขึ้นเอง (Proprietary Security Selection Methodology) โดยเน้นลงทุนในบริษัทที่เป็นผู้นำด้านผลิตภัณฑ์หน่วยความจำเซมิคอนดักเตอร์และเทคโนโลยีที่เกี่ยวข้อง โดยพิจารณาปัจจัยต่างๆ เช่น ส่วนแบ่งทางการตลาดและส่วนแบ่งรายได้ที่มาจากการขายหรือการผลิตผลิตภัณฑ์ดังกล่าว
กองทุนจะลงทุนในกองทุนหลักด้วยสกุลเงินดอลลาร์สหรัฐ (USD) และกองทุนหลักจะนำเงินไปลงทุนในหลักทรัพย์หรือทรัพย์สินในสกุลเงินดอลลาร์สหรัฐ (USD) และอาจลงทุนในหลักทรัพย์หรือทรัพย์สินที่ซื้อขายหรือมีมูลค่าอ้างอิงเป็นสกุลเงินอื่นได้ ตามประเทศหรือภูมิภาคที่กองทุนหลักเข้าไปลงทุน
ในส่วนที่เหลือ กองทุนจะพิจารณาลงทุนในตราสารทุน ตราสารหนี้ และ/หรือตราสารกึ่งหนี้กึ่งทุน และ/หรือตราสารทางการเงิน และ/หรือเงินฝาก และ/หรือหน่วย CIS และ/หรือหน่วยลงทุนกองทุนรวมอสังหาริมทรัพย์/REITs และ/หรือหน่วยลงทุนกองทุนรวมโครงสร้างพื้นฐาน และ/หรือหน่วยลงทุนกองทุนสินค้าโภคภัณฑ์ และ/หรือกองทุนอีทีเอฟ (ETF) และ/หรือหน่วยลงทุน private equity และ/หรือทรัพย์สินอื่นใดที่มีลักษณะทำนองเดียวกัน โดยได้รับความเห็นชอบจากสำนักงานคณะกรรมการ ก.ล.ต. หรือคณะกรรมการกำกับตลาดทุนประกาศกำหนดให้กองทุนลงทุนได้ ทั้งนี้ จะมีการลงทุนที่ส่งผลให้มี Net Exposure ที่เกี่ยวข้องกับความเสี่ยงต่างประเทศโดยเฉลี่ยในรอบปีบัญชี
ไม่น้อยกว่าร้อยละ 80 ของมูลค่าทรัพย์สินสุทธิของกองทุน 
นอกจากนี้ กองทุนอาจมีการลงทุนในสัญญาซื้อขายล่วงหน้า (Derivatives) โดยมีวัตถุประสงค์เพื่อลดความเสี่ยง (Hedging) ซึ่งกองทุนจะเข้าเป็นคู่สัญญาโดยมีสินทรัพย์อ้างอิงอย่างใดอย่างหนึ่งหรือหลายอย่าง ได้แก่ หุ้น อัตราดอกเบี้ย อัตราแลกเปลี่ยนเงิน เครดิต (Credit Rating และ/หรือ Credit Event) ซึ่งหากราคาสินทรัพย์อ้างอิงมีความผันผวนมากหรือทิศทางการลงทุนไม่เป็นไปตามที่คาดการณ์ หรือคู่สัญญาที่ทําธุรกรรมไม่สามารถชําระภาระผูกพันได้ตามกําหนดเวลา อาจทําให้กองทุนขาดทุนจากการลงทุนในสัญญาซื้อขายล่วงหน้า ทั้งนี้ ผู้จัดการกองทุนอาจพิจารณาปรับเปลี่ยนการลงทุนหรือการเข้าทําธุรกรรมให้เหมาะสมกับสถานการณ์เพื่อให้เกิดประโยชน์ต่อกองทุน และ/หรือผู้ถือหน่วยลงทุนโดยรวม อย่างไรก็ตาม ผู้จัดการกองทุนจะวิเคราะห์และติดตามปัจจัยที่มีผลกระทบต่อราคาสินทรัพย์อ้างอิง รวมถึงพิจารณาความน่าเชื่อถือของคู่สัญญาก่อนทําธุรกรรมและติดตามอย่างต่อเนื่อง โดยกองทุนจะเน้นทําธุรกรรมกับคู่สัญญาที่มีอันดับความน่าเชื่อถืออยู่ในอันดับที่สามารถลงทุนได้ (Investment Grade)

สำหรับการลงทุนในต่างประเทศ กองทุนอาจทำการป้องกันความเสี่ยง (Hedging) จากอัตราแลกเปลี่ยนเงินตราต่างประเทศ โดยขึ้นอยู่กับดุลยพินิจของผู้จัดการกองทุน (Dynamic Hedging) ตามความเหมาะสมสำหรับสภาวการณ์ในแต่ละขณะ โดยพิจารณาปัจจัยที่เกี่ยวข้อง เช่น ปัจจัยทางเศรษฐกิจ สภาวะของตลาดการเงิน สภาวะการลงทุนในขณะนั้น แนวโน้มและความผันผวนของอัตราแลกเปลี่ยนและต้นทุนในการป้องกันความเสี่ยง เป็นต้น ทั้งนี้ กองทุนอาจป้องกันความเสี่ยงบางส่วนหรือทั้งจำนวนหรืออาจไม่ป้องกันความเสี่ยงก็ได้ตามความเหมาะสม การใช้เครื่องมือป้องกันความเสี่ยงอาจมีต้นทุนจากการทำธุรกรรมซึ่งอาจส่งผลให้ผลตอบแทนของกองทุนโดยรวมลดลง
ทั้งนี้ กองทุนอาจทำธุรกรรมการให้ยืมหลักทรัพย์ (Securities Lending) หรือธุรกรรมการซื้อโดยมีสัญญาขายคืน (Reverse Repo) โดยเป็นไปตามหลักเกณฑ์ที่สำนักงานค …(ตัดทอน)

### ลักษณะเฉพาะของโครงการ

- `FED` กองทุนรวมฟีดเดอร์ (Feeder Fund )
- `SE` กองทุนรวมหมวดอุตสาหกรรม (Sector Fund)

## 4. ความเสี่ยง

**ระดับความเสี่ยง: 7 / 8** — 7 — หมวดอุตสาหกรรม

> ลงทุนในตราสารทุน โดยเน้นเพียงบางหมวดอุตสาหกรรม โดยเฉลี่ยในรอบปีไม่น้อยกว่า 80% ของ NAV

อ่านเพิ่ม: [[ระดับความเสี่ยงกองทุนรวม]]

## 5. ค่าธรรมเนียม

ดูคำอธิบายแต่ละประเภทที่ [[ค่าธรรมเนียมกองทุนรวม]]

### สรุปต่อชนิดหน่วยลงทุน (Share Class)

| ชนิดหน่วยลงทุน | ซื้อได้โดย | รวม: เก็บจริง (%) | รวม: เพดาน (%) | จัดการ: เก็บจริง (%) | ขาย (%) | รับซื้อคืน (%) |
|---|---|---|---|---|---|---|
| `ASP-MEM` | ⚪ ไม่ระบุ | 2.64 | 3.99 | 1.61 | 1.5 | - |

**เก็บจริง** = อัตราที่เรียกเก็บในรอบที่รายงาน · **เพดาน** = อัตราสูงสุดที่หนังสือชี้ชวนอนุญาต — สองค่านี้เทียบกันข้ามกองไม่ได้

อธิบายชนิดหน่วยลงทุนที่ [[ชนิดหน่วยลงทุน Share Class]] · [[../Indexes/compare-fees|เทียบกับกองอื่นในหมวดเดียวกัน]]

### ตาม Factsheet ล่าสุด (ทุกแถวที่รายงาน)

| Class | ประเภท | เพดาน (%) | เก็บจริง (%) |
|---|---|---|---|
| ASP-MEM | ค่าธรรมเนียมการขายหน่วยลงทุน (Front-end Fee) | 1.61 | 1.5 |
| ASP-MEM | ค่าธรรมเนียมการจัดการ (Management Fee) | 1.61 | 1.61 |
| ASP-MEM | ค่าธรรมเนียมการโอนหน่วยลงทุน (Transfer Fee) | 50 | 50 |
| ASP-MEM | ค่าธรรมเนียมและค่าใช้จ่ายรวมทั้งหมด (Total Fee and Expense) | 3.99 | 2.64 |
| ASP-MEM | ค่าธรรมเนียมการสับเปลี่ยนหน่วยลงทุนเข้า (SWITCHING IN) | 1.61 | 1.5 |

### ตามหนังสือชี้ชวน (เพดานตามโครงการ)

| Class | ประเภท | อัตรา | หน่วย |
|---|---|---|---|
| main | ค่าธรรมเนียมการจัดการ (Management Fee) | 1.61 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมทั้งหมด (Total Fee) | 3.99 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมที่ปรึกษาการลงทุน (Investment Advisor Fee) | - | - |
| main | ค่าธรรมเนียมนายทะเบียนหน่วยลงทุน (Registrar Fee) | 1 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมผู้จัดจำหน่ายหน่วยลงทุน (Distributor Fee) | - | - |
| main | ค่าธรรมเนียมผู้ดูแลผลประโยชน์ (Trustee Fee) | 0.08 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมอื่น ๆ (Other Fee) | 1.3 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |

## 6. ผลการดำเนินงาน

_ไม่มีข้อมูลผลการดำเนินงานจาก API_

### ดัชนีชี้วัด (Benchmark)

| # | ดัชนีชี้วัด | หมายเหตุ |
|---|---|---|
| 1 | ผลการดำเนินงานของกองทุนรวมหลัก | ผลการดำเนินงานของกองทุนรวมหลัก ในสัดส่วน 20% โดยปรับด้วยอัตราแลกเปลี่ยนเพื่อคำนวณผลตอบแทนเป็นสกุลเงินบาท ณ วันที่คำนวณผลตอบแทน โดยมีวัตถุประสงค์เพื่อเปรียบเทียบกับผลการดำเนินงานของกองทุนรวม (Performance Objective) |
| 2 | ผลการดำเนินงานของกองทุนรวมหลัก | ผลการดำเนินงานของกองทุนรวมหลัก ในสัดส่วน 80% โดยปรับด้วยต้นทุนการป้องกันความเสี่ยงด้านอัตราแลกเปลี่ยนเพื่อคำนวณผลตอบแทนเป็นสกุลเงินบาท ณ วันที่คำนวณผลตอบแทน |

## 7. พอร์ตการลงทุน

### ⚖️ ปัจจัยที่กระทบกอง (สองด้าน)

> [!INFO] ปัจจัยที่กองนี้**ไว**ต่อ — อ่านจากหมวด/ประเทศ/โครงสร้างที่ถือจริง
> แสดง**ทั้งสองด้าน** (ขึ้น=โอกาส · ลง=ความเสี่ยง) · **ไม่ใช่การพยากรณ์** ว่าจะไปทางไหน · [กรอบข้อมูลอ้างอิง](../../docs/project/ideas.md)

| ปัจจัย | หมวด | ถ้าขึ้น ▲ | ถ้าลง ▼ | จาก |
|---|---|---|---|---|
| ค่าธรรมเนียมรวมสูง (2.64%/ปี) | โครงสร้าง | - | ค่าธรรมเนียมยิ่งสูง ยิ่งกินผลตอบแทนทุกปีไม่ว่าตลาดขึ้นหรือลง | โครงสร้างกอง |


### 📊 เคลื่อนไหวสัมพันธ์กับอะไร (correlation อดีต)

| ปัจจัย | correlation | ทิศทาง | จำนวนวัน |
|---|---|---|---|
| หุ้นตลาดเกิดใหม่ (EM) | +0.68 | ไปทางเดียวกัน | 32 |

> **วัดจาก NAV รายวัน** ช่วง 2026-07-08..2026-08-26 (ที่มา: NAV × Yahoo) · +1 = ไปทางเดียวกัน · −1 = สวนทาง
> [!warning] เป็น**ความสัมพันธ์ที่วัดจากอดีตในช่วงข้างต้น** — **ไม่ใช่การพยากรณ์** · correlation ไม่ใช่สาเหตุ และ**ไม่นิ่ง** (มักพุ่งเข้า 1 ตอนตลาดวิกฤต)
### การจัดสรรสินทรัพย์ (จาก Factsheet)

| สินทรัพย์ | สัดส่วน (%) |
|---|---|
| Net Exposure ในหน่วยลงทุนของ กองทุนรวมต่างประเทศ โดยเฉลี่ยในรอบปีบัญชีไม่น้อยกว่าร้อยละ | 80 |

### 5 อันดับแรกที่ลงทุน

| # | หลักทรัพย์ | สัดส่วน (%) |
|---|---|---|
| 1 | มี Net Exposure ในกองทุน Roundhill Memory ETF โดยเฉลี่ย ในรอบปีบัญชีไม่น้อยกว่าร้อยละ | 80 |

### สัดส่วนตามประเภทสินทรัพย์ (งวด 202607)

| รหัส | ประเภทสินทรัพย์ | มูลค่าตลาด | % NAV |
|---|---|---|---|
| 108 | หน่วยลงทุน (รหัส 108-109,117-121,139,130) | 71,460,766 | 80.695 |
| 101 | หุ้น (รหัส 101-102) | 11,624,163 | 13.1263 |
| 201 | เงินฝากธนาคาร P/N และ B/E (รหัส 202-205,216-224) | 5,190,492 | 5.8612 |
| 401 | ตราสารอนุพันธ์(รหัส 401-407) | 329,757 | 0.3724 |
| 700 | สินทรัพย์อื่นๆ/หนี้สินอื่นๆ (รหัส 122,211,301-399,500-599) หัก (รหัส 601-699) | -48,559 | -0.0548 |

### ข้อมูลเพิ่มเติมจาก Factsheet

**ทรัพย์สินที่ลงทุนสูงสุดของกองทุนหลัก (look-through)**

| ทรัพย์สิน | % NAV |
|---|---|
| Samsung Electronics Co | 25.18 |
| SK hynix | 24.76 |
| Micron Technology Inc | 24.76 |
| Sandisk | 4.99 |
| Kioxia Holdings | 4.6 |

> รายละเอียดทั้งหมดดูที่ [[Factsheet - ASP-MEM]]

## 8. NAV

| Class | วันที่ | NAV/หน่วย | ราคาขาย | ราคารับซื้อคืน | มูลค่าทรัพย์สินสุทธิ |
|---|---|---|---|---|---|
| main | 2026-08-26 | 8.8227 | 8.9551 | 8.8227 | 117,890,225 |

อ่านเพิ่ม: [[NAV และราคาซื้อขายหน่วยลงทุน]]

### NAV ย้อนหลัง 2026-07-08 → 2026-08-26 (ชนิด `main`)

`███▅▇▅▃▃▃▆▆▆▃▂▂▁▂▄▃▁▁▁▁▅▅▇▄▄▅▅▃▄▄`

| ช่วง | วันที่ | จำนวนวัน | เปลี่ยนแปลง | ความผันผวน (ต่อปี) |
|---|---|---|---|---|
| ทั้งช่วง | 2026-07-08 → 2026-08-26 | 33 | -11.77% | 79.5% |

สูงสุด 10 · ต่ำสุด 7.7816 (ตลอดช่วงที่เก็บ)

> ตัวเลข**คำนวณเองจาก NAV รายวัน** (ที่มา: `data/raw/nav.jsonl`) · เป็นข้อมูล**ย้อนหลัง** ไม่ใช่การพยากรณ์ · ผลตอบแทน/ผันผวนอดีตไม่รับประกันอนาคต · ช่วงที่กองอายุยังไม่ถึงจะไม่แสดง

## 9. เงินปันผล

| Class | นโยบายปันผล |
|---|---|
| ASP-MEM | ไม่จ่ายเงินปันผล |

## 10. การซื้อขายหน่วยลงทุน

### ยอดซื้อขายขั้นต่ำ

| Class | มีผลตั้งแต่ | ซื้อขั้นต่ำ (IPO) | สกุล (IPO) | ซื้อขั้นต่ำ | สกุล | ขายคืนขั้นต่ำ | สกุล (ขายคืน) | ยอดคงเหลือขั้นต่ำ | สกุล (คงเหลือ) |
|---|---|---|---|---|---|---|---|---|---|
| ASP-MEM | 2026-07-06 | 1000.0 | THB | 1000.0 | THB | - | THB | - | THB |

### ช่วงเวลาซื้อขายและการชำระเงิน

| Class | มีผลตั้งแต่ | ประเภทรายการ | ช่วงเวลาทำรายการ | รายละเอียดเพิ่มเติม | ระยะเวลารับเงิน |
|---|---|---|---|---|---|
| ASP-MEM | 2026-07-06 | การซื้อ (subscription) | อื่น ๆ โปรดระบุ | IPO ระหว่างวันที่ 6 - 8 ก.ค. 69 หลัง IPO : เริ่มวันที่ 10 ก.ค. 69 เป็นต้นไป  ทุกวันทำการซื้อขาย ทั้งนี้ หากมีการเปลี่ยนแปลงระยะเวลาการซื้อช่วง IPO หรือ หลัง IPO จะแจ้งให้ทราบผ่านเว็บไซต์ | - |
| ASP-MEM | 2026-07-06 | การขายคืน (redemption) | อื่น ๆ โปรดระบุ | เริ่มวันที่ 10 ก.ค. 69 เป็นต้นไป  ทุกวันทำการซื้อขาย | T+5 คือ5 วันทำการนับจากวันคำนวณ NAV |

## 11. บุคคลที่เกี่ยวข้อง

### ผู้จัดการกองทุน (จาก Factsheet)

- นาย กมลยศ สุขุมสุวรรณ
- น.ส. ภัทรลภา รัตนดิลก ณ ภูเก็ต

| บทบาท | ชื่อ |
|---|---|
| นายทะเบียนหน่วยลงทุน | บริษัทหลักทรัพย์จัดการกองทุน แอสเซท พลัส จำกัด |
| T | บริษัทหลักทรัพย์จัดการกองทุน แอสเซท พลัส จำกัด |

## 12. Factsheet

📄 ข้อความที่แกะจาก PDF: [[Factsheet - ASP-MEM]]

- class `ASP-MEM` (ณ 2026-07-06) — [PDF](https://secdocumentstorage.blob.core.windows.net/fundfactsheet/M0696_2569.pdf)

---

## ดูเพิ่ม

- กองอื่นของ [[บริษัทหลักทรัพย์จัดการกองทุน แอสเซท พลัส จำกัด]]
- กองประเภทเดียวกัน: [[ตราสารทุน]]
- ความเสี่ยงระดับเดียวกัน: [[by-risk|กองความเสี่ยงระดับ 7]]
- กองทุนหลักที่ลงทุนจริง: [[Roundhill Memory ETF]]
- [[00-home|🏠 Home]] · [[all-funds|รายชื่อกองทุนทั้งหมด]]

> ข้อมูลจาก SEC Open API · อัปเดตล่าสุด `2026-08-27T07:42:31.783`
