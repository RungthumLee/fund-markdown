---
title: "X-DATACENEQ"
proj_id: M1026_2568
regis_id: "MF0637_2568"
abbr: "X-DATACENEQ"
name_th: "กองทุนเปิดเอ็กซ์สปริงศูนย์ข้อมูล และโครงสร้างพื้นฐานดิจิทัล อิควิตี้"
name_en: "XSpring Data Center and Digital Infrastructure Equity"
amc: "บริษัทหลักทรัพย์จัดการกองทุน เอ็กซ์สปริง จำกัด"
amc_id: "C0000006099"
policy: "ตราสารทุน"
risk_spectrum: "6"
management_style: "PN"
retail_type: "R"
invest_country_flag: "1"
init_date: "2025-11-11"
regis_date: "2025-11-28"
class_count: 1
has_factsheet: true
ter_retail: 1.171
perf_1y: 0.0
nav: 13.5466
nav_date: "2026-08-26"
fund_size: 118120915.61
holdings_count: 3
top10_pct_nav: 99.77
market_countries: ["สหรัฐฯ", "ออสเตรเลีย", "เกาหลีใต้"]
country_top: "สหรัฐฯ"
country_coverage_pct: 66.1
master_fund: "Global X Data Center & Digital Infrastructure ETF"
peer_group: "Global Equity - Infrastructure"
fund_managers: ["นาย เมธวัฒน์ ว่องกิจ", "นาง สาว นฤมล มีมาก"]
tags: [fund, sec-data, asset/equity, risk/high, liquidity/t1, sector/real-estate, cap/large, fx/discretionary, struct/feeder, style/passive, use/accumulate]
---

# X-DATACENEQ

**กองทุนเปิดเอ็กซ์สปริงศูนย์ข้อมูล และโครงสร้างพื้นฐานดิจิทัล อิควิตี้**  
_XSpring Data Center and Digital Infrastructure Equity_

บลจ. [[บริษัทหลักทรัพย์จัดการกองทุน เอ็กซ์สปริง จำกัด]] · นโยบาย [[ตราสารทุน]] · ความเสี่ยงระดับ **6**

> [!abstract] สรุปย่อ
> - **รหัสโครงการ:** `M1026_2568`
> - **ประเภท:** ตราสารทุน · Feeder — กองหลัก passive
> - **ผู้ลงทุน:** ผู้ลงทุนทั่วไป
> - **ต่างประเทศ:** เน้นลงทุนต่างประเทศ
> - **จัดตั้ง:** 2025-11-11 · **จดทะเบียน:** 2025-11-28
> - **ชนิดหน่วยลงทุน:** 1 class
> - **NAV 13.5466 (2026-08-26)**

> [!tip] อ่านง่าย ๆ
> กองหุ้น · ความเสี่ยงสูง (6/8) · เหมาะกับสะสมระยะยาว · ขายคืนแล้วได้เงินภายใน T+1 · เน้นกลุ่มอสังหา/REIT · ลงทุนต่างประเทศ (สหรัฐฯ) · ป้องกันค่าเงินตามดุลยพินิจผู้จัดการ · ลงทุนผ่านกองทุนหลัก (feeder) · บริหารแบบอิงดัชนี (passive) · ค่าธรรมเนียมรวมที่รายย่อยจ่ายจริงราว 1.17%/ปี

## 1. ข้อมูลทั่วไป

| รายการ | ค่า |
|---|---|
| เลขที่โครงการ (proj_id) | `M1026_2568` |
| เลขที่จดทะเบียน (regis_id) | MF0637_2568 |
| ชื่อย่อ | X-DATACENEQ |
| ชื่อไทย | กองทุนเปิดเอ็กซ์สปริงศูนย์ข้อมูล และโครงสร้างพื้นฐานดิจิทัล อิควิตี้ |
| ชื่ออังกฤษ | XSpring Data Center and Digital Infrastructure Equity |
| บลจ. | [[บริษัทหลักทรัพย์จัดการกองทุน เอ็กซ์สปริง จำกัด]] |
| สถานะ | Registered |
| วันจัดตั้ง | 2025-11-11 |
| วันจดทะเบียน | 2025-11-28 |
| ประเภทตามนโยบาย | ตราสารทุน |
| กลยุทธ์บริหาร | PN — Feeder — กองหลัก passive |
| ลักษณะโครงการ | R — ผู้ลงทุนทั่วไป |
| ความเสี่ยงต่างประเทศ | เน้นลงทุนต่างประเทศ |

### กองทุนหลัก (Feeder Fund)

- **กองทุนหลัก:** Global X Data Center & Digital Infrastructure ETF
- **ประเทศ:** สหรัฐอเมริกา
- 🌐 **โน้ตกองทุนหลัก:** [[Global X Data Center & Digital Infrastructure ETF]] — ค่าธรรมเนียม ขนาดกองทุน sector และหลักทรัพย์ที่ถือจริง

ดูแนวคิดที่ [[Feeder Fund]]

### นโยบายป้องกันความเสี่ยงอัตราแลกเปลี่ยน

ดุลยพินิจ (dynamic hedging) (0%-105% ของมูลค่าความเสี่ยง)

## 2. ชนิดหน่วยลงทุน (Share Class)

| Class | ISIN | สิทธิประโยชน์ภาษี | รายละเอียด |
|---|---|---|---|
| main | - | - | - |

## 3. นโยบายการลงทุน

กองทุนเน้นลงทุนในหน่วยลงทุนของ Global X Data Center & Digital Infrastructure ETF(กองทุนหลัก) เพียงกองทุนเดียวในสกุลเงินดอลลาร์สหรัฐ (USD) โดยเฉลี่ยในรอบปีบัญชีไม่น้อยกว่าร้อยละ 80 ของมูลค่าทรัพย์สินสุทธิของกองทุน ทั้งนี้ กองทุนหลักดังกล่าวเป็นกองทุนประเภท Exchange Traded Fund (ETF) ที่บริหารจัดการโดย Solactive AG ที่มีการจดทะเบียนซื้อขายหน่วยลงทุนในตลาดหลักทรัพย์ NASDAQ ประเทศสหรัฐอเมริกา โดยกองทุนหลักจัดตั้งตามกฎหมายของประเทศสหรัฐอเมริกาซึ่งเป็นสมาชิกสามัญของ International Organizations of Securities Commission (IOSCO)
กองทุนหลักมุ่งเน้นที่จะสร้างผลตอบแทนจากการลงทุนก่อนหักค่าธรรมเนียมและค่าใช้จ่ายให้ได้ใกล้เคียงกับผลการดำเนินงานของดัชนีอ้างอิงซึ่งได้แก่ ดัชนี Solactive Data Center REITs & Digital Infrastructure Index (ดัชนีอ้างอิง) ภายใต้สถานการณ์ปกติ โดยกองทุนหลักมุ่งเน้นการลงทุนในหุ้นบริษัททั่วโลกที่ทำธุรกิจที่เกี่ยวข้องกับธุรกิจศูนย์ข้อมูล (Data Center) เสาสัญญาณโทรศัพท์ และ/หรือโครงสร้างพื้นฐานของดิจิทัล 
กองทุนหลักจดทะเบียนซื้อขายอยู่ในตลาดหลักทรัพย์ NASDAQ ของประเทศสหรัฐอเมริกา ในสกุลเงินดอลลาร์สหรัฐ บริษัทจัดการขอสงวนสิทธิที่จะพิจารณาเปลี่ยนแปลงตลาดซื้อขายหลักทรัพย์อื่นนอกเหนือจากตลาดหลักทรัพย์ NASDAQ เช่น ตลาดหลักทรัพย์นิวยอร์ก ตลาดหลักทรัพย์ยูโรเน็กซ์ เป็นต้น และ/หรือเปลี่ยนแปลงสกุลเงินที่กองทุนนำไปลงทุนได้ โดยบริษัทจัดการจะคำนึงถึงประโยชน์ของผู้ถือหน่วยลงทุนเป็นสำคัญ และไม่ถือเป็นการดำเนินการที่ผิดไปจากรายละเอียดโครงการ โดยถือว่าได้รับความเห็นชอบจากผู้ถือหน่วยลงทุนแล้ว
สำหรับการลงทุนในส่วนที่เหลือจากการลงทุนในกองทุนหลัก กองทุนอาจพิจารณาลงทุนในหรือมีไว้ซึ่งตราสารทุน ตราสารหนี้ ตราสารกึ่งหนี้กึ่งทุน เงินฝาก บัตรเงินฝาก ตราสารเทียบเท่าเงินฝาก หน่วย CIS หน่วยProperty REITs หน่วย Infra และ/หรือหน่วยอีทีเอฟ (ETF) รวมถึงกองทุนอาจทำธุรกรรมการให้ยืมหลักทรัพย์ และ/หรือธุรกรรมการซื้อโดยมีสัญญาขายคืน (Reverse Repo) โดยเป็นไปตามหลักเกณฑ์ที่คณะกรรมการ ก.ล.ต. คณะกรรมการกำกับตลาดทุนและสำนักงานคณะกรรมการ ก.ล.ต. ประกาศกำหนด รวมทั้งลงทุนในหลักทรัพย์และ/หรือทรัพย์สินอื่นใดที่เสนอขายทั้งในประเทศและ/หรือต่างประเทศ และ/หรือการหาดอกผลตามหลักเกณฑ์ที่สำนักงานคณะกรรมการ ก.ล.ต. กำหนดหรือเห็นชอบให้ลงทุนได้
กองทุนอาจลงทุนในหน่วยลงทุนของกองทุนรวมอื่น กองทุนรวมอสังหาริมทรัพย์ ทรัสต์เพื่อการลงทุนในอสังหาริมทรัพย์ (REITs) กองทุนรวมโครงสร้างพื้นฐาน ซึ่งอยู่ภายใต้การจัดการของบริษัทจัดการในสัดส่วนโดยเฉลี่ยในรอบปีบัญชีไม่เกินร้อยละ 100 ของมูลค่าทรัพย์สินสุทธิของกองทุน โดยการลงทุนในหน่วยลงทุนของกองทุนรวมอื่นดังกล่าวต้องอยู่ภายใต้กรอบนโยบายการลงทุนของกองทุน ทั้งนี้ กองทุนรวมอื่นซึ่งอยู่ภายใต้การจัดการของบริษัทจัดการสามารถลงทุนในหน่วยลงทุนของกองทุนปลายทางต่อไปอีกได้ไม่เกิน 1 ทอด และกองทุนปลายทางนั้นไม่สามารถลงทุนย้อนกลับในกองทุนต้นทาง (Circle investment) ได้ และ/หรือตามหลักเกณฑ์และเงื่อนไขที่สำนักงานคณะกรรมการ ก.ล.ต. กำหนด
กองทุนอาจพิจารณาลงทุนในกองทุนหลักโดยใช้สกุลเงินดอลลาร์สหรัฐฯ (USD) เป็นสกุลเงินหลักหรือสกุลเงินอื่นใดตามที่บริษัทจัดการเห็นสมควรหากมีการเปลี่ยนแปลงโดยกองทุนหลักในอนาคต ทั้งนี้ กองทุนหลักอาจลงทุนได้หลายสกุลเงิน เช่น สกุลเงินหยวน (RMB) สกุลเงินดอลลาร์สหรัฐ (USD) สกุลยูโร (EUR) เป็นต้น ในการนี้บริษัทจัดการขอสงวนสิทธิที่จะเปลี่ยนแปลงสกุลเงินที่จะลงทุนในกองทุนหลักในภายหลัง โดยถือว่าได้รับความเห็นชอบจากผู้ถือหน่วยลงทุนแล้ว ซึ่งบริษัทจัดการจะคำนึงถึงประโยชน์ของกองทุนเป็นสำคัญ โดยบริษัทจัดการจะแจ้งให้ผู้ถือหน่วยลงทุนทราบผ่านทางเว็บไซต์ของบริษัทจัดการหรือช่องทางอื่นที่เหมาะสม
กองทุนอาจลงทุนหรือมีไว้ซึ่งสัญญาซื้อขายล่วงหน้า (Derivatives) เพื่อลดความเสี่ยง (Hedging) ทั้งนี้ สัญญาซื้อขายล่วงหน้าที่กองทุนจะเข้าเป็นคู่สัญญาจะมีสินทรัพย์อ้างอิงอย่างใดอย่างหนึ่งหรือหลายอย่าง ได้แก่ อัตราดอกเบี้ย อัตราแลกเปลี่ยนเงิน หากราคาสินทรัพย์อ้างอิงมีความผันผวนมากหรือทิศทางการลงทุนไม่เป็นไปตามที่คาดการณ์ หรือคู่สัญญาที่ทำธุรกรรมไม่สามารถชำระภาระผูกพันได้ตามกำหนดเวลา อาจทำให้กองทุนขาดทุนจากการลงทุนในสัญญาซื้อขายล่วงหน้าได้ ทั้งนี้ บริษัทจัดการอาจพิจารณาปรับเปลี่ยนการลงทุนหรือเข้าทำธุรกรรมให้เหมาะสมกับสถานการณ์ เพื่อให้เกิดประโยชน์สูงสุดต่อผู้ถือหน่วยลงทุนโดยรวม อย่างไรก็ตาม บริษัทจัดการจะวิเคราะห์และติดตามปัจจัยที่มีผลกระทบต่อราคาสินทรัพย์อ้างอิง รวมถึงพิจารณาความน่าเชื่อถือของคู่สัญญาก่อนทำธุรกรรมและติดตามอย่างต่อเนื่อง โดยกองทุนจะเน้นทำธุรกรรมกับคู่สัญญาที่มีอันด …(ตัดทอน)

### ลักษณะเฉพาะของโครงการ

- `CIV` กองทุนรวมที่มีนโยบายเปิดให้มีการลงทุนในกองทุนรวมอื่นภายใต้ บลจ. เดียวกัน (CROSS Investing Fund)
- `FED` กองทุนรวมฟีดเดอร์ (Feeder Fund )

## 4. ความเสี่ยง

**ระดับความเสี่ยง: 6 / 8** — 6 — ตราสารทุน

> ลงทุนในตราสารทุนทั่วโลก โดยเฉลี่ยในรอบปีไม่น้อยกว่า
ร้อยละ 80 ของมูลค่าทรัพย์สินสุทธิ

อ่านเพิ่ม: [[ระดับความเสี่ยงกองทุนรวม]]

### สถิติเชิงปริมาณ

| Class | Sharpe | Alpha | Beta | Max Drawdown | Tracking Error | Turnover | FX Hedging |
|---|---|---|---|---|---|---|---|
| main | - | - | - | -8.60% | - | 3.83 | - |

อ่านเพิ่ม: [[สถิติวัดผลกองทุน]]

## 5. ค่าธรรมเนียม

ดูคำอธิบายแต่ละประเภทที่ [[ค่าธรรมเนียมกองทุนรวม]]

### สรุปต่อชนิดหน่วยลงทุน (Share Class)

| ชนิดหน่วยลงทุน | ซื้อได้โดย | รวม: เก็บจริง (%) | รวม: เพดาน (%) | จัดการ: เก็บจริง (%) | ขาย (%) | รับซื้อคืน (%) |
|---|---|---|---|---|---|---|
| `main` | ⚪ ไม่ระบุ | 1.171 | 4 | 1.07 | 1.605 | - |

**เก็บจริง** = อัตราที่เรียกเก็บในรอบที่รายงาน · **เพดาน** = อัตราสูงสุดที่หนังสือชี้ชวนอนุญาต — สองค่านี้เทียบกันข้ามกองไม่ได้

อธิบายชนิดหน่วยลงทุนที่ [[ชนิดหน่วยลงทุน Share Class]] · [[../Indexes/compare-fees|เทียบกับกองอื่นในหมวดเดียวกัน]]

### ตาม Factsheet ล่าสุด (ทุกแถวที่รายงาน)

| Class | ประเภท | เพดาน (%) | เก็บจริง (%) |
|---|---|---|---|
| main | ค่าธรรมเนียมและค่าใช้จ่ายรวมทั้งหมด (Total Fee and Expense) | 4 | 1.171 |
| main | ค่าธรรมเนียมการขายหน่วยลงทุน (Front-end Fee) | 2.14 | 1.605 |
| main | ค่าธรรมเนียมการจัดการ (Management Fee) | 2.14 | 1.07 |

### ตามหนังสือชี้ชวน (เพดานตามโครงการ)

| Class | ประเภท | อัตรา | หน่วย |
|---|---|---|---|
| main | ค่าธรรมเนียมการจัดการ (Management Fee) | 2.14 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมทั้งหมด (Total Fee) | 4 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมที่ปรึกษาการลงทุน (Investment Advisor Fee) | - | - |
| main | ค่าธรรมเนียมนายทะเบียนหน่วยลงทุน (Registrar Fee) | 0.321 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมผู้จัดจำหน่ายหน่วยลงทุน (Distributor Fee) | - | - |
| main | ค่าธรรมเนียมผู้ดูแลผลประโยชน์ (Trustee Fee) | 0.321 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |
| main | ค่าธรรมเนียมอื่น ๆ (Other Fee) | 1.218 | ต่อปี ของมูลค่าทรัพย์สินสุทธิของกองทุนรวม |

## 6. ผลการดำเนินงาน

### Class `main`

| รายการ | 3 เดือน | 6 เดือน | YTD | 1 ปี | 5 ปี | 10 ปี | ตั้งแต่จัดตั้ง | 2025 | 2024 | 2023 | 2022 | 2021 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| ผลตอบแทนกองทุนรวม | 26.13 | 47.83 | 47.83 | 0 | 0 | 0 | 47.47 | -0.52 | 0 | 0 | 0 | 0 |
| ผลตอบแทนตัวชี้วัด | 29.31 | 52.37 | 52.37 | - | - | - | 52.19 | - | - | - | - | - |
| ค่าเฉลี่ยในกลุ่มเดียวกัน | 1.76 | 9.75 | 9.75 | - | - | - | - | - | - | - | - | - |
| ความผันผวนของกองทุนรวม | 28.09 | 26.45 | 26.45 | - | - | - | 26.24 | - | - | - | - | - |
| ความผันผวนของตัวชี้วัด | 25.49 | 24.17 | 24.17 | - | - | - | 23.86 | - | - | - | - | - |

> ตัวเลขเป็น % ต่อปี · เทียบ **ผลตอบแทนกองทุนรวม** กับ **ผลตอบแทนตัวชี้วัด** และ **ค่าเฉลี่ยในกลุ่มเดียวกัน** เสมอ  
> ดู [[สถิติวัดผลกองทุน]] · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## 7. พอร์ตการลงทุน

### การกระจายตามประเทศ (ตลาดจริงจากหลักทรัพย์)

| ประเทศ (ตลาดที่ซื้อขาย) | % ของกอง |
|---|---|
| สหรัฐฯ | 53.2 |
| ออสเตรเลีย | 5 |
| เกาหลีใต้ | 4.1 |
| สิงคโปร์ | 3.8 |

> ทะลุถึงหลักทรัพย์จริงได้ **66.1%** ของกอง (ส่วนที่เหลือ 33.9% ยังไม่ทะลุ) · ที่มา: look-through (Yahoo top holdings) · symbol/ISIN

### ⚖️ ปัจจัยที่กระทบกอง (สองด้าน)

> [!INFO] ปัจจัยที่กองนี้**ไว**ต่อ — อ่านจากหมวด/ประเทศ/โครงสร้างที่ถือจริง
> แสดง**ทั้งสองด้าน** (ขึ้น=โอกาส · ลง=ความเสี่ยง) · **ไม่ใช่การพยากรณ์** ว่าจะไปทางไหน · [กรอบข้อมูลอ้างอิง](../../docs/project/ideas.md)

| ปัจจัย | หมวด | ถ้าขึ้น ▲ | ถ้าลง ▼ | จาก |
|---|---|---|---|---|
| ดอกเบี้ยเฟด / เศรษฐกิจสหรัฐ | Macro | เศรษฐกิจแข็งแรง/เฟดผ่อนคลาย | เฟดขึ้นดอกเบี้ย/เศรษฐกิจชะลอ | ประเทศ: สหรัฐฯ |


### 📊 เคลื่อนไหวสัมพันธ์กับอะไร (correlation อดีต)

| ปัจจัย | correlation | ทิศทาง | จำนวนวัน |
|---|---|---|---|
| หุ้นตลาดเกิดใหม่ (EM) | +0.72 | ไปทางเดียวกัน | 169 |
| หุ้นสหรัฐ (S&P 500) | +0.64 | ไปทางเดียวกัน | 169 |

> **วัดจาก NAV รายวัน** ช่วง 2025-11-28..2026-08-26 (ที่มา: NAV × Yahoo) · +1 = ไปทางเดียวกัน · −1 = สวนทาง
> [!warning] เป็น**ความสัมพันธ์ที่วัดจากอดีตในช่วงข้างต้น** — **ไม่ใช่การพยากรณ์** · correlation ไม่ใช่สาเหตุ และ**ไม่นิ่ง** (มักพุ่งเข้า 1 ตอนตลาดวิกฤต)
### การจัดสรรสินทรัพย์ (จาก Factsheet)

| สินทรัพย์ | สัดส่วน (%) |
|---|---|
| ทรัพย์สินและหนี้สินอื่น | 0.24 |
| เงินฝากธนาคาร | 2.85 |
| - | - |
| หน่วยลงทุน | 96.91 |
| ตราสารหนี้ | - |

### 5 อันดับแรกที่ลงทุน

| # | หลักทรัพย์ | สัดส่วน (%) |
|---|---|---|
| 1 | Data Center & Digital Infrastructure ETF | 96.91 |
| 2 | - | 0 |
| 3 | - | 0 |
| 4 | - | 0 |
| 5 | - | 0 |

### สัดส่วนตามประเภทสินทรัพย์ (งวด 202607)

| รหัส | ประเภทสินทรัพย์ | มูลค่าตลาด | % NAV |
|---|---|---|---|
| 101 | หุ้น (รหัส 101-102) | 75,673,716 | 102.413 |
| 201 | เงินฝากธนาคาร P/N และ B/E (รหัส 202-205,216-224) | 2,576,714 | 3.4872 |
| 700 | สินทรัพย์อื่นๆ/หนี้สินอื่นๆ (รหัส 122,211,301-399,500-599) หัก (รหัส 601-699) | -4,359,726 | -5.9002 |

### รายการลงทุนรายตัวทั้งหมด (งวด 202606 · ณ 2026-06-30)

- จำนวนรายการที่ถือ: **3** รายการ จากผู้ออก 2 ราย
- มูลค่าทรัพย์สินสุทธิของกองทุน: **68,334,451** บาท
- น้ำหนัก 10 อันดับแรกรวม: **99.77% ของ NAV** (คิดเป็น 100.0% ของพอร์ต)

| # | หลักทรัพย์ | ผู้ออก | ประเภท | ISIN | มูลค่า | % NAV |
|---|---|---|---|---|---|---|
| 1 | [[../Entities/Global X Data Center & Digital\|Global X Data Center & Digital]] <br><sub>`DTCR US`</sub> | DCTR US | หุ้นสามัญ | - | 66,224,616 | 96.9125 |
| 2 | [[../Entities/BAY\|BAY]] <br><sub>`SA-X-DATACENEQ`</sub> | BAY | เงินฝากธนาคารประเภทออมทรัพย์ | - | 1,948,264 | 2.8511 |
| 3 | [[../Entities/BAY\|BAY]] <br><sub>`CA-X-DATACENEQ`</sub> | BAY | เงินฝากธนาคารประเภทกระแสรายวัน | - | 2, | 0.0029 |

### 🔭 ทะลุกองทุนหลัก (Look-through)

กองนี้ถือ **Global X Data Center & Digital Infrastructure ETF** อยู่ **96.91%** ของ NAV ตารางด้านล่างคูณต่อด้วยสัดส่วนที่กองทุนหลักถือหลักทรัพย์แต่ละตัว

> [!CAUTION] ตัวเลขนี้เป็น **ขั้นต่ำ** ไม่ใช่สัดส่วนที่แท้จริง
> ใช้เฉพาะหลักทรัพย์ **10 อันดับแรก** ที่กองทุนหลักเปิดเผย รวมกันได้เพียง **66.13%** จาก 96.91% ที่กองถืออยู่
> ส่วนที่เหลืออยู่ในหลักทรัพย์ที่ไม่ได้เปิดเผยรายตัว และวันอ้างอิงของสองฝั่งไม่ตรงกัน
> อ่าน [[Look-through การถือทางอ้อม|ข้อจำกัดฉบับเต็ม]] ก่อนใช้ตัดสินใจ

| # | หลักทรัพย์ | Symbol | % ของกองทุนหลัก | ~% ของกองนี้ |
|---|---|---|---|---|
| 1 | [[../Entities/EQIX US Equity\|Equinix Inc]] | EQIX | 13.4 | 12.9863 |
| 2 | [[../Entities/Digital Realty\|Digital Realty Trust Inc]] | DLR | 12.43 | 12.0462 |
| 3 | [[../Entities/AMT US Equity\|American Tower Corp]] | AMT | 10.26 | 9.9432 |
| 4 | [[../Entities/Crown Castle Inc\|Crown Castle Inc]] | CCI | 6.78 | 6.5707 |
| 5 | [[../Entities/NEXTDC LTD\|Nextdc Ltd]] | NXT.AX | 5.2 | 5.0394 |
| 6 | [[../Entities/Micron Technology Inc\|Micron Technology Inc]] | MU | 4.31 | 4.1769 |
| 7 | [[../Entities/SK Hynix Inc\|SK hynix Inc]] | 000660.KQ | 4.21 | 4.08 |
| 8 | [[../Entities/MARVELL TECHNOLOGY GROUP LTD\|Marvell Technology Inc]] | MRVL | 4.04 | 3.9153 |
| 9 | [[../Entities/Keppel DC REIT\|Keppel DC REIT]] | AJBU.SI | 3.89 | 3.7699 |
| 10 | [[../Entities/ADVANCED MICRO DEVICES\|Advanced Micro Devices Inc]] | AMD | 3.72 | 3.6051 |

[[../Indexes/by-lookthrough|ดัชนีการถือทางอ้อมทั้งหมด]] · [[ค่าธรรมเนียมสองชั้นของ Feeder Fund]]

### ข้อมูลเพิ่มเติมจาก Factsheet

**ทรัพย์สินที่ลงทุนสูงสุดของกองทุนหลัก (look-through)**

| ทรัพย์สิน | % NAV |
|---|---|
| Equinix Inc | 12.38 |
| Digital Realty Trust Inc | 10.71 |
| American Tower Corp | 8.74 |
| Crown Castle Inc | 6.08 |
| Marvell Technology Inc | 5.81 |
| United States | 75.42 |
| China | 7.1 |
| Australia | 5.92 |
| South Korea | 4.88 |
| Singapore | 3.74 |
| Real Estate | 51.55 |
| Information Technology | 45.67 |

> รายละเอียดทั้งหมดดูที่ [[Factsheet - X-DATACENEQ]]

## 8. NAV

| Class | วันที่ | NAV/หน่วย | ราคาขาย | ราคารับซื้อคืน | มูลค่าทรัพย์สินสุทธิ |
|---|---|---|---|---|---|
| main | 2026-08-26 | 13.5466 | 13.7641 | 13.5466 | 118,120,916 |

อ่านเพิ่ม: [[NAV และราคาซื้อขายหน่วยลงทุน]]

### NAV ย้อนหลัง 2025-11-28 → 2026-08-26 (ชนิด `main`)

`▁▁▁▁▁▁▂▂▂▃▃▃▄▄▃▄▃▃▄▃▃▄▄▅▆▇▇▇▇██▇███▆▆▆▆▆▆▆▇▆▆`

| ช่วง | วันที่ | จำนวนวัน | เปลี่ยนแปลง | ความผันผวน (ต่อปี) |
|---|---|---|---|---|
| ทั้งช่วง | 2025-11-28 → 2026-08-26 | 177 | 35.6% | 26.5% |

สูงสุด 15.5434 · ต่ำสุด 9.4203 (ตลอดช่วงที่เก็บ)

> ตัวเลข**คำนวณเองจาก NAV รายวัน** (ที่มา: `data/raw/nav.jsonl`) · เป็นข้อมูล**ย้อนหลัง** ไม่ใช่การพยากรณ์ · ผลตอบแทน/ผันผวนอดีตไม่รับประกันอนาคต · ช่วงที่กองอายุยังไม่ถึงจะไม่แสดง

## 9. เงินปันผล

| Class | นโยบายปันผล |
|---|---|
| main | ไม่จ่ายเงินปันผล |

## 10. การซื้อขายหน่วยลงทุน

### ยอดซื้อขายขั้นต่ำ

| Class | มีผลตั้งแต่ | ซื้อขั้นต่ำ (IPO) | สกุล (IPO) | ซื้อขั้นต่ำ | สกุล | ขายคืนขั้นต่ำ | สกุล (ขายคืน) | ยอดคงเหลือขั้นต่ำ | สกุล (คงเหลือ) |
|---|---|---|---|---|---|---|---|---|---|
| main | 2026-06-30 | 5000.0 | THB | - | THB | - | THB | 1000.0 | THB |

### ช่วงเวลาซื้อขายและการชำระเงิน

| Class | มีผลตั้งแต่ | ประเภทรายการ | ช่วงเวลาทำรายการ | รายละเอียดเพิ่มเติม | ระยะเวลารับเงิน |
|---|---|---|---|---|---|
| main | 2026-06-30 | การซื้อ (subscription) | ทุกวันทำการ | ทุกวันทำการ | - |
| main | 2026-06-30 | การขายคืน (redemption) | ทุกวันทำการ | ทุกวันทำการ | T+1 คือ1 วันทำการนับจากวันคำนวณ NAV |

## 11. บุคคลที่เกี่ยวข้อง

### ผู้จัดการกองทุน (จาก Factsheet)

- นาย เมธวัฒน์ ว่องกิจ
- นาง สาว นฤมล มีมาก

| บทบาท | ชื่อ |
|---|---|
| D | บริษัทหลักทรัพย์จัดการกองทุน เอ็กซ์สปริง จำกัด |
| นายทะเบียนหน่วยลงทุน | บริษัทหลักทรัพย์จัดการกองทุน เอ็กซ์สปริง จำกัด |
| T | บริษัทหลักทรัพย์จัดการกองทุน เอ็กซ์สปริง จำกัด |

## 12. Factsheet

📄 ข้อความที่แกะจาก PDF: [[Factsheet - X-DATACENEQ]]

- class `main` (ณ 2026-06-30) — [PDF](https://secdocumentstorage.blob.core.windows.net/fundfactsheet/M1026_2568.pdf)

---

## ดูเพิ่ม

- กองอื่นของ [[บริษัทหลักทรัพย์จัดการกองทุน เอ็กซ์สปริง จำกัด]]
- กองประเภทเดียวกัน: [[ตราสารทุน]]
- ความเสี่ยงระดับเดียวกัน: [[by-risk|กองความเสี่ยงระดับ 6]]
- กองทุนหลักที่ลงทุนจริง: [[Global X Data Center & Digital Infrastructure ETF]]
- [[00-home|🏠 Home]] · [[all-funds|รายชื่อกองทุนทั้งหมด]]

> ข้อมูลจาก SEC Open API · อัปเดตล่าสุด `2026-08-27T07:42:31.783`
