---
title: "Schroder ISF Glb Clmt Chg Eq C Acc USD"
master_key: "LU0302446132"
isin: "LU0302446132"
quote_type: "MUTUALFUND"
category: "Sector Equity Ecology"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "1.03%"
feeder_count: 2
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 Schroder ISF Glb Clmt Chg Eq C Acc USD

**ISIN:** `LU0302446132` · **Ticker:** `0P00009BWH`

กองทุนหลักของกองทุนไทย **2** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | Schroder ISF Glb Clmt Chg Eq C Acc USD |
| ISIN | `LU0302446132` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | Sector Equity Ecology |
| ผู้จัดการกองทุน | Simon Webber (เริ่ม 29 Jun 2007) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2021-01-14 |
| นโยบายปันผล | Accumulation |
| สไตล์การลงทุน | Market Cap: Large |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 1.57bn GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **1.03%** |
| ค่าธรรมเนียมแรกเข้า (initial charge) | 1.00% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 8.76% |
| 1 ปี (FT) | +18.56% |
| Beta (3 ปี) | 1.22 |
| Morningstar rating | ★★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 99.31% |
| เงินสด | 0.69% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| อุตสาหกรรม | 30.63% |
| เทคโนโลยี | 30.26% |
| สินค้าฟุ่มเฟือย | 12.59% |
| สาธารณูปโภค | 8.49% |
| สื่อสาร | 7.25% |
| สินค้าจำเป็น | 4.27% |
| วัตถุดิบ | 2.92% |
| การเงิน | 2.33% |
| อสังหาริมทรัพย์ | 1.27% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | สัดส่วน |
|---|---|---|---|---|
| 1 | [[../Entities/Alphabet Inc Class A\|Alphabet Inc Class A]] | `GOOGL` | สหรัฐฯ | 7.20% |
| 2 | [[../Entities/MICROSOFT CORP\|Microsoft Corp]] | `MSFT` | สหรัฐฯ | 4.33% |
| 3 | [[../Entities/AMAZON.COM INC\|Amazon.com Inc]] | `AMZN` | สหรัฐฯ | 3.70% |
| 4 | [[../Entities/Infineon Technologies AG\|Infineon Technologies AG]] | `IFX.DE` | เยอรมนี | 3.24% |
| 5 | [[../Entities/Vestas Wind Systems AS\|Vestas Wind Systems AS]] | `VWS.CO` | - | 2.90% |
| 6 | [[../Entities/Keyence Corp\|Keyence Corp]] | `6861.T` | ญี่ปุ่น | 2.67% |
| 7 | [[../Entities/Contemporary Amperex Technology Co Ltd Class A\|Contemporary Amperex Technology Co Ltd Class A]] | `300750.SZ` | จีน | 2.66% |
| 8 | [[../Entities/IBE SM Equity\|Iberdrola SA]] | `IBE.MC` | - | 2.62% |
| 9 | [[../Entities/NEE US Equity\|NextEra Energy Inc]] | `NEE` | สหรัฐฯ | 2.60% |
| 10 | [[../Entities/SCHNEIDER ELECTRIC SE\|Schneider Electric SE]] | `SU.PA` | ฝรั่งเศส | 2.43% |

## กองทุนไทยที่ลงทุนในกองนี้

**2 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[KT-CLIMATE]] | [[บริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)]] | 6 | 97.77% | 1.54 | **≈ 2.57** |
| [[KT-CLIMATE RMF]] | [[บริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน)]] | 6 | 97.60% | - | - |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**1.03%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

> [!NOTE]
> กองไทยทั้ง 2 กองนี้ลงทุนในกองทุนหลัก**เดียวกัน** การถือหลายกองจึงไม่ได้กระจายความเสี่ยงอย่างที่คิด

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P00009BWH`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU0302446132:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
