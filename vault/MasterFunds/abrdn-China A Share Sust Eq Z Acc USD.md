---
title: "abrdn-China A Share Sust Eq Z Acc USD"
master_key: "LU1130125955"
isin: "LU1130125955"
quote_type: "MUTUALFUND"
category: "China Equity - A Shares"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "0.13%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 abrdn-China A Share Sust Eq Z Acc USD

**ISIN:** `LU1130125955` · **Ticker:** `0P00018MSH`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | abrdn-China A Share Sust Eq Z Acc USD |
| ISIN | `LU1130125955` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | China Equity - A Shares |
| ผู้จัดการกองทุน | Nicholas Yeo (เริ่ม 16 Mar 2015) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2022-03-03 |
| นโยบายปันผล | Accumulation |
| สไตล์การลงทุน | Market Cap: Large |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 610.14m GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.13%** |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 9.25% |
| 1 ปี (FT) | +22.10% |
| Beta (3 ปี) | 1.0 |
| Morningstar rating | ★★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 98.57% |
| เงินสด | 1.43% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| เทคโนโลยี | 33.64% |
| อุตสาหกรรม | 20.48% |
| การเงิน | 17.32% |
| วัตถุดิบ | 9.76% |
| สินค้าฟุ่มเฟือย | 6.75% |
| สุขภาพ | 5.35% |
| สินค้าจำเป็น | 3.72% |
| สาธารณูปโภค | 1.72% |
| พลังงาน | 1.27% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | สัดส่วน |
|---|---|---|---|---|
| 1 | [[../Entities/Contemporary Amperex Technology Co Ltd Class A\|Contemporary Amperex Technology Co Ltd Class A]] | `300750.SZ` | จีน | 6.67% |
| 2 | [[../Entities/NAURA Technology Group Co Ltd Class A\|NAURA Technology Group Co Ltd Class A]] | `002371.SZ` | จีน | 5.41% |
| 3 | [[../Entities/Zhongji Innolight Co Ltd Class A\|Zhongji Innolight Co Ltd Class A]] | `300308.SZ` | จีน | 4.94% |
| 4 | Luxshare Precision Industry Co Ltd Class A | `002475.SZ` | จีน | 4.80% |
| 5 | [[../Entities/Wus Printed Circuit (Kunshan) Co Ltd Class A\|Wus Printed Circuit (Kunshan) Co Ltd Class A]] | `002463.SZ` | จีน | 3.94% |
| 6 | [[../Entities/Kweichow Moutai Co Ltd Class A\|Kweichow Moutai Co Ltd Class A]] | `600519.SS` | จีน | 3.51% |
| 7 | [[../Entities/China Merchants Bank Co Ltd Class A\|China Merchants Bank Co Ltd Class A]] | `600036.SS` | จีน | 3.01% |
| 8 | [[../Entities/CMOC Group Ltd Class A\|CMOC Group Ltd Class A]] | `603993` | - | 2.75% |
| 9 | [[../Entities/Sieyuan Electric Co Ltd Class A\|Sieyuan Electric Co Ltd Class A]] | `002028.SZ` | จีน | 2.68% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[ABCA]] | [[บริษัทหลักทรัพย์จัดการกองทุน อเบอร์ดีน (ประเทศไทย) จำกัด]] | 6 | 102.43% | 2.07 | **≈ 2.20** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.13%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P00018MSH`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU1130125955:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
