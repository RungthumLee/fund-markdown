---
title: "abrdn- Global Smaller Comp Z Acc USD"
master_key: "LU2530219000"
isin: "LU2530219000"
quote_type: "MUTUALFUND"
category: "Global Small/Mid-Cap Equity"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "0.13%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 abrdn- Global Smaller Comp Z Acc USD

**ISIN:** `LU2530219000` · **Ticker:** `0P0001PYGF`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | abrdn- Global Smaller Comp Z Acc USD |
| ISIN | `LU2530219000` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | Global Small/Mid-Cap Equity |
| ผู้จัดการกองทุน | Kirsty Desson (เริ่ม 25 Nov 2022) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2026-04-21 |
| นโยบายปันผล | Accumulation |
| สไตล์การลงทุน | Market Cap: Mid |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 86.05m GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.13%** |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | -0.24% |
| 1 ปี (FT) | -1.61% |
| Beta (3 ปี) | 1.04 |
| Morningstar rating | ★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 95.42% |
| เงินสด | 4.44% |
| ตราสารหนี้ | 0.14% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| อุตสาหกรรม | 26.46% |
| สินค้าฟุ่มเฟือย | 16.99% |
| เทคโนโลยี | 14.07% |
| สุขภาพ | 13.67% |
| การเงิน | 13.57% |
| พลังงาน | 6.31% |
| สินค้าจำเป็น | 3.77% |
| อสังหาริมทรัพย์ | 3.66% |
| สื่อสาร | 0.84% |
| วัตถุดิบ | 0.66% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | สัดส่วน |
|---|---|---|---|---|
| 1 | [[../Entities/Wintrust Financial Corp\|Wintrust Financial Corp]] | `WTFC` | สหรัฐฯ | 4.76% |
| 2 | Lattice Semiconductor Corp | `LSCC` | สหรัฐฯ | 3.77% |
| 3 | Encompass Health Corp | `EHC` | สหรัฐฯ | 3.74% |
| 4 | [[../Entities/Gaztransport et technigaz SA\|Gaztransport et technigaz SA]] | `GTT.PA` | ฝรั่งเศส | 3.67% |
| 5 | [[../Entities/Casey's General Stores Inc\|Casey's General Stores Inc]] | `CASY` | สหรัฐฯ | 3.57% |
| 6 | SharkNinja Inc | `SN` | สหรัฐฯ | 3.48% |
| 7 | [[../Entities/ALS Ltd\|ALS Ltd]] | `ALQ.AX` | ออสเตรเลีย | 2.76% |
| 8 | [[../Entities/Ligand Pharmaceuticals Inc\|Ligand Pharmaceuticals Inc]] | `LGND` | สหรัฐฯ | 2.68% |
| 9 | Advanced Drainage Systems Inc | `WMS` | สหรัฐฯ | 2.66% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[ABGS-M]] | [[บริษัทหลักทรัพย์จัดการกองทุน อเบอร์ดีน (ประเทศไทย) จำกัด]] | 6 | 100.60% | 2.09 | **≈ 2.22** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.13%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P0001PYGF`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU2530219000:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
