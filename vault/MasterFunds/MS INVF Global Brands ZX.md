---
title: "MS INVF Global Brands ZX"
master_key: "LU0360612351"
isin: "LU0360612351"
quote_type: "MUTUALFUND"
category: "Global Large-Cap Blend Equity"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "0.86%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 MS INVF Global Brands ZX

**ISIN:** `LU0360612351` · **Ticker:** `0P0000NBBU`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | MS INVF Global Brands ZX |
| ISIN | `LU0360612351` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | Global Large-Cap Blend Equity |
| ผู้จัดการกองทุน | William D. Lock (เริ่ม 22 Jun 2009) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2011-11-11 |
| นโยบายปันผล | Income |
| สไตล์การลงทุน | Market Cap: Large |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 7.53bn GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.86%** |
| ค่าธรรมเนียมแรกเข้า (initial charge) | 3.00% |
| Yield | 0.31% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | -6.95% |
| 1 ปี (FT) | -7.94% |
| Beta (3 ปี) | 0.68 |
| Morningstar rating | ★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 98.11% |
| เงินสด | 1.64% |
| ตราสารหนี้ | 0.48% |
| อื่น ๆ | -0.22% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| การเงิน | 22.73% |
| เทคโนโลยี | 17.50% |
| สินค้าจำเป็น | 15.06% |
| สุขภาพ | 14.47% |
| สินค้าฟุ่มเฟือย | 10.54% |
| สื่อสาร | 9.95% |
| อุตสาหกรรม | 9.76% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | กลุ่ม | สัดส่วน |
|---|---|---|---|---|---|
| 1 | [[../Entities/MICROSOFT CORP\|Microsoft Corp]] | `MSFT` | สหรัฐฯ | เทคโนโลยี | 5.24% |
| 2 | [[../Entities/KO US Equity\|Coca-Cola Co]] | `KO` | สหรัฐฯ | ผู้บริโภค | 5.18% |
| 3 | [[../Entities/Alphabet Inc Class A\|Alphabet Inc Class A]] | `GOOGL` | สหรัฐฯ | สื่อสาร | 5.08% |
| 4 | [[../Entities/Netflix Inc\|Netflix Inc]] | `NFLX` | สหรัฐฯ | สื่อสาร | 4.68% |
| 5 | [[../Entities/Visa Inc Class A\|Visa Inc Class A]] | `V` | สหรัฐฯ | การเงิน | 4.58% |
| 6 | [[../Entities/L'Oreal SA\|L'Oreal SA]] | `OR.PA` | ฝรั่งเศส | ผู้บริโภค | 4.34% |
| 7 | [[../Entities/Taiwan Semiconductor Manufacturing Co Ltd\|Taiwan Semiconductor Manufacturing Co Ltd]] | `2330.TW` | ไต้หวัน | เทคโนโลยี | 4.13% |
| 8 | [[../Entities/Aon PLC Class A\|Aon PLC Class A]] | `AON` | สหรัฐฯ | การเงิน | 4.05% |
| 9 | [[../Entities/Intercontinental Exchange Inc\|Intercontinental Exchange Inc]] | `ICE` | สหรัฐฯ | การเงิน | 4.02% |
| 10 | [[../Entities/RELX PLC\|RELX PLC]] | `REL.L` | สหราชอาณาจักร | อุตสาหกรรม | 3.46% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[KFGBRANSSF]] | [[บริษัทหลักทรัพย์จัดการกองทุน กรุงศรี จำกัด]] | 6 | 100.98% | 1.54 | **≈ 2.40** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.86%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P0000NBBU`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU0360612351:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
