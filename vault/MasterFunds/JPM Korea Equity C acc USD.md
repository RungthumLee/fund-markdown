---
title: "JPM Korea Equity C acc USD"
master_key: "LU0301638002"
isin: "LU0301638002"
quote_type: "MUTUALFUND"
category: "Korea Equity"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "0.95%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 JPM Korea Equity C acc USD

**ISIN:** `LU0301638002` · **Ticker:** `0P0000CUQ8`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | JPM Korea Equity C acc USD |
| ISIN | `LU0301638002` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | Korea Equity |
| ผู้จัดการกองทุน | John Cho (เริ่ม 13 Jun 2011) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2020-09-17 |
| นโยบายปันผล | Accumulation |
| สไตล์การลงทุน | Market Cap: Large |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 3.23bn GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.95%** |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 39.91% |
| 1 ปี (FT) | +107.86% |
| Beta (3 ปี) | 0.79 |
| Morningstar rating | ★★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 104.82% |
| เงินสด | -4.82% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| เทคโนโลยี | 45.69% |
| อุตสาหกรรม | 20.22% |
| การเงิน | 11.97% |
| สินค้าฟุ่มเฟือย | 8.08% |
| สุขภาพ | 4.84% |
| วัตถุดิบ | 4.35% |
| สินค้าจำเป็น | 1.45% |
| พลังงาน | 1.37% |
| สื่อสาร | 1.09% |
| สาธารณูปโภค | 0.93% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | สัดส่วน |
|---|---|---|---|---|
| 1 | [[../Entities/SK Hynix Inc\|SK hynix Inc]] | `000660.KQ` | เกาหลีใต้ | 11.75% |
| 2 | [[../Entities/Samsung Electronics Co Ltd\|Samsung Electronics Co Ltd]] | `005930.KQ` | เกาหลีใต้ | 10.91% |
| 3 | [[../Entities/SK Square\|SK Square]] | `402340.KQ` | เกาหลีใต้ | 9.02% |
| 4 | [[../Entities/Samsung C&T Corp\|Samsung C&T Corp]] | `028260.KQ` | เกาหลีใต้ | 4.79% |
| 5 | [[../Entities/Samsung Electro-Mechanics Co Ltd\|Samsung Electro-Mechanics Co Ltd]] | `009150.KQ` | เกาหลีใต้ | 4.72% |
| 6 | [[../Entities/SK Inc\|SK Inc]] | `034730.KQ` | เกาหลีใต้ | 3.50% |
| 7 | [[../Entities/Shinhan Financial Group Co Ltd\|Shinhan Financial Group Co Ltd]] | `055550.KQ` | เกาหลีใต้ | 3.17% |
| 8 | [[../Entities/HD Hyundai Heavy Industries\|HD Hyundai Heavy Industries]] | `329180.KQ` | เกาหลีใต้ | 2.81% |
| 9 | [[../Entities/KB Financial Group Inc\|KB Financial Group Inc]] | `105560.KQ` | เกาหลีใต้ | 2.75% |
| 10 | [[../Entities/Samsung Life Insurance Co Ltd\|Samsung Life Insurance Co Ltd]] | `032830.KQ` | เกาหลีใต้ | 2.36% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[DAOL-KOREAEQ]] | [[บริษัทหลักทรัพย์จัดการกองทุน ดาโอ จำกัด]] | 6 | 102.72% | 1.98 | **≈ 2.93** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.95%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P0000CUQ8`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU0301638002:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
