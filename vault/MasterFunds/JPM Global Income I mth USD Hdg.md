---
title: "JPM Global Income I mth USD Hdg"
master_key: "LU2293733585"
isin: "LU2293733585"
quote_type: "MUTUALFUND"
category: "USD Moderate Allocation"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "0.72%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 JPM Global Income I mth USD Hdg

**ISIN:** `LU2293733585` · **Ticker:** `0P0001LSIF`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | JPM Global Income I mth USD Hdg |
| ISIN | `LU2293733585` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | USD Moderate Allocation |
| ผู้จัดการกองทุน | Michael Schoenhaut (เริ่ม 11 Dec 2008) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2018-06-28 |
| นโยบายปันผล | Income |
| สไตล์การลงทุน | Market Cap: Large |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 16.05bn GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.72%** |
| Yield | 5.56% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 6.65% |
| 1 ปี (FT) | +6.67% |
| Beta (3 ปี) | 0.88 |
| Morningstar rating | ★★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| ตราสารหนี้ | 47.87% |
| หุ้น | 44.98% |
| เงินสด | 6.26% |
| หุ้นบุริมสิทธิ | 0.62% |
| หุ้นกู้แปลงสภาพ | 0.30% |
| อื่น ๆ | 0.26% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| เทคโนโลยี | 26.09% |
| การเงิน | 18.27% |
| อุตสาหกรรม | 8.77% |
| สินค้าฟุ่มเฟือย | 7.87% |
| สื่อสาร | 7.81% |
| สุขภาพ | 7.81% |
| พลังงาน | 6.04% |
| สาธารณูปโภค | 5.45% |
| สินค้าจำเป็น | 5.24% |
| อสังหาริมทรัพย์ | 3.50% |
| วัตถุดิบ | 3.16% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | สัดส่วน |
|---|---|---|---|---|
| 1 | [[../Entities/JPM Nasdaq Eq Prm Inc Act ETF USD Dist\|JPM Nasdaq Eq Prm Inc Act ETF USD Dist]] | `JEPQ.L` | สหราชอาณาจักร | 6.36% |
| 2 | [[../Entities/MICROSOFT CORP\|Microsoft Corp]] | `MSFT` | สหรัฐฯ | 1.14% |
| 3 | [[../Entities/Taiwan Semiconductor Manufacturing Co Ltd\|Taiwan Semiconductor Manufacturing Co Ltd]] | `2330.TW` | ไต้หวัน | 1.04% |
| 4 | [[../Entities/NVIDIA Corp\|NVIDIA Corp]] | `NVDA` | สหรัฐฯ | 0.74% |
| 5 | [[../Entities/ASML HOLDING NV\|ASML Holding NV]] | `ASML.AS` | เนเธอร์แลนด์ | 0.65% |
| 6 | [[../Entities/Alphabet Inc Class A\|Alphabet Inc Class A]] | `GOOGL` | สหรัฐฯ | 0.50% |
| 7 | [[../Entities/Mastercard Inc Class A\|Mastercard Inc Class A]] | `MA` | สหรัฐฯ | 0.47% |
| 8 | [[../Entities/Volvo AB Class B\|Volvo AB Class B]] | `VOLV-B.ST` | - | 0.40% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[KF-INCOME]] | [[บริษัทหลักทรัพย์จัดการกองทุน กรุงศรี จำกัด]] | 5 | 98.79% | 1.27 | **≈ 1.99** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.72%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P0001LSIF`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU2293733585:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
