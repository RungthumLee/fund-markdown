---
title: "iShares Edge MSCI World Value Factor UCITS ETF USD (Acc)"
master_key: "IE00BP3QZB59"
isin: "IE00BP3QZB59"
quote_type: "ETF"
fund_family: "BlackRock Asset Management Ireland - ETF"
currency: "USD"
ongoing_charge: "0.30%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data, etf]
---

# 🌐 iShares Edge MSCI World Value Factor UCITS ETF USD (Acc)

**ISIN:** `IE00BP3QZB59` · **Ticker:** `IWVL.L`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | iShares Edge MSCI World Value Factor UCITS ETF USD (Acc) |
| ISIN | `IE00BP3QZB59` |
| ประเภท | ETF (จดทะเบียนซื้อขายในตลาด) |
| โครงสร้างทางกฎหมาย | Exchange Traded Fund |
| ประเทศจดทะเบียน | ไอร์แลนด์ |
| บริษัทจัดการ | BlackRock Asset Management Ireland - ETF |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2018-02-23 |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (Yahoo) | 7.21 พันล้าน USD |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.30%** |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 30.03% |
| 3 ปี (เฉลี่ยต่อปี) | 29.70% |
| 5 ปี (เฉลี่ยต่อปี) | 17.28% |
| Beta (3 ปี) | 1.17 |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 99.17% |
| เงินสด | 0.57% |
| อื่น ๆ | 0.26% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| เทคโนโลยี | 29.96% |
| การเงิน | 16.28% |
| อุตสาหกรรม | 11.80% |
| สินค้าฟุ่มเฟือย | 9.09% |
| สุขภาพ | 8.72% |
| สื่อสาร | 8.16% |
| สินค้าจำเป็น | 5.05% |
| พลังงาน | 4.00% |
| วัตถุดิบ | 2.79% |
| สาธารณูปโภค | 2.42% |
| อสังหาริมทรัพย์ | 1.74% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | สัดส่วน |
|---|---|---|---|---|
| 1 | [[../Entities/Micron Technology Inc\|Micron Technology Inc]] | `MU` | สหรัฐฯ | 12.53% |
| 2 | [[../Entities/CISCO SYSTEMS, INC\|Cisco Systems Inc]] | `CSCO` | สหรัฐฯ | 3.24% |
| 3 | [[../Entities/Verizon Communications Inc\|Verizon Communications Inc]] | `VZ` | สหรัฐฯ | 2.14% |
| 4 | [[../Entities/Toyota Motor Corp\|Toyota Motor Corp]] | `7203.T` | ญี่ปุ่น | 1.74% |
| 5 | [[../Entities/AT&T, INC\|AT&T Inc]] | `T` | สหรัฐฯ | 1.59% |
| 6 | Comcast Corp Class A | `CMCSA` | สหรัฐฯ | 1.36% |
| 7 | [[../Entities/QUALCOMM, INC\|Qualcomm Inc]] | `QCOM` | สหรัฐฯ | 1.25% |
| 8 | Hewlett Packard Enterprise Co | `HPE` | สหรัฐฯ | 1.23% |
| 9 | [[../Entities/GENERAL MOTORS CO\|General Motors Co]] | `GM` | สหรัฐฯ | 1.19% |
| 10 | [[../Entities/BATS LN Equity\|British American Tobacco PLC]] | `BATS.L` | สหราชอาณาจักร | 1.16% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[SCBGVALUE]] | [[บริษัทหลักทรัพย์จัดการกองทุน ไทยพาณิชย์ จำกัด]] | 6 | 99.84% | 0.11 | **≈ 0.41** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.30%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `IWVL.L`)
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
