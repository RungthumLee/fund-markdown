---
title: "Invesco WilderHill Clean Energy ETF"
master_key: "US46137V1347"
isin: "US46137V1347"
quote_type: "ETF"
category: "Small Growth"
fund_family: "Invesco"
currency: "USD"
ongoing_charge: "0.64%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data, etf]
---

# 🌐 Invesco WilderHill Clean Energy ETF

**ISIN:** `US46137V1347` · **Ticker:** `PBW`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | Invesco WilderHill Clean Energy ETF |
| ISIN | `US46137V1347` |
| ประเภท | ETF (จดทะเบียนซื้อขายในตลาด) |
| โครงสร้างทางกฎหมาย | Exchange Traded Fund |
| ประเทศจดทะเบียน | สหรัฐอเมริกา |
| หมวด (Morningstar) | Small Growth |
| บริษัทจัดการ | Invesco |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2005-03-03 |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (Yahoo) | 412.58 ล้าน USD |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.64%** |
| Yield | 1.47% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 5.82% |
| 3 ปี (เฉลี่ยต่อปี) | -0.48% |
| 5 ปี (เฉลี่ยต่อปี) | -14.28% |
| Beta (3 ปี) | 2.13 |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 99.82% |
| เงินสด | 0.18% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| เทคโนโลยี | 26.74% |
| อุตสาหกรรม | 25.18% |
| วัตถุดิบ | 18.76% |
| สินค้าฟุ่มเฟือย | 15.44% |
| สาธารณูปโภค | 11.81% |
| สินค้าจำเป็น | 2.08% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | กลุ่ม | สัดส่วน |
|---|---|---|---|---|---|
| 1 | OPAL Fuels Inc Ordinary Shares - Class A | `OPAL` | สหรัฐฯ | สาธารณูปโภค | 2.41% |
| 2 | BETA Technologies Inc Ordinary Shares- Class A | `BETA` | สหรัฐฯ | อุตสาหกรรม | 2.20% |
| 3 | [[../Entities/Itron Inc\|Itron Inc]] | `ITRI` | สหรัฐฯ | เทคโนโลยี | 2.17% |
| 4 | Gentherm Inc Class A | `THRM` | สหรัฐฯ | ผู้บริโภค | 2.10% |
| 5 | [[../Entities/Darling Ingredients Inc\|Darling Ingredients Inc]] | `DAR` | สหรัฐฯ | ผู้บริโภค | 2.08% |
| 6 | Axia Energia ADR | `AXIA` | สหรัฐฯ | สาธารณูปโภค | 1.95% |
| 7 | Cadeler AS ADR | `CDLR` | สหรัฐฯ | อุตสาหกรรม | 1.91% |
| 8 | Gevo Inc | `GEVO` | สหรัฐฯ | วัสดุ | 1.90% |
| 9 | Canadian Solar Inc | `CSIQ` | สหรัฐฯ | เทคโนโลยี | 1.80% |
| 10 | Rivian Automotive Inc Class A | `RIVN` | สหรัฐฯ | ผู้บริโภค | 1.79% |

## คำอธิบายกองทุน (ต้นฉบับภาษาอังกฤษ)

> The fund generally will invest at least 90% of its total assets in the securities that comprise the underlying index. The index is composed of stocks of publicly traded companies listed on a major exchange in the U.S. that are engaged in the business of the advancement of cleaner energy and conservation or are important to the development of clean energy. Stocks are included in the index based on the index provider's evaluation that such companies will substantially benefit from a societal transition toward the use of cleaner energy, zero-CO2 renewables and conservation.

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[TNEWENGY]] | [[บริษัทหลักทรัพย์จัดการกองทุน ทิสโก้]] | 6 | 99.00% | 1.37 | **≈ 2.01** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.64%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `PBW`)
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
