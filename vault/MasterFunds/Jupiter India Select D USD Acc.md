---
title: "Jupiter India Select D USD Acc"
master_key: "LU0946219929"
isin: "LU0946219929"
quote_type: "MUTUALFUND"
category: "India Equity"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "0.95%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 Jupiter India Select D USD Acc

**ISIN:** `LU0946219929` · **Ticker:** `0P00016CTF`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | Jupiter India Select D USD Acc |
| ISIN | `LU0946219929` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | India Equity |
| ผู้จัดการกองทุน | Avinash Vazirani (เริ่ม 02 May 2008) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2024-03-07 |
| นโยบายปันผล | Accumulation |
| สไตล์การลงทุน | Market Cap: Large |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 635.32m GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.95%** |
| ค่าธรรมเนียมแรกเข้า (initial charge) | 5.00% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | -9.34% |
| 1 ปี (FT) | -7.66% |
| Beta (3 ปี) | 0.97 |
| Morningstar rating | ★★★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 99.07% |
| เงินสด | 0.79% |
| ตราสารหนี้ | 0.12% |
| หุ้นกู้แปลงสภาพ | 0.02% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| การเงิน | 31.23% |
| สุขภาพ | 14.10% |
| สินค้าจำเป็น | 11.55% |
| อุตสาหกรรม | 10.70% |
| พลังงาน | 9.60% |
| สินค้าฟุ่มเฟือย | 7.79% |
| สื่อสาร | 6.28% |
| เทคโนโลยี | 3.99% |
| อสังหาริมทรัพย์ | 2.91% |
| วัตถุดิบ | 1.03% |
| สาธารณูปโภค | 0.82% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | กลุ่ม | สัดส่วน |
|---|---|---|---|---|---|
| 1 | Godfrey Phillips India Ltd | `500163` | - | - | 6.51% |
| 2 | [[../Entities/State Bank of India\|State Bank of India]] | `SBIN.NS` | อินเดีย | การเงิน | 5.75% |
| 3 | [[../Entities/Fortis Healthcare Ltd\|Fortis Healthcare Ltd]] | `FORTIS.BO` | อินเดีย | สุขภาพ | 5.16% |
| 4 | [[../Entities/Bharti Airtel Ltd\|Bharti Airtel Ltd]] | `BHARTIARTL.NS` | อินเดีย | สื่อสาร | 4.83% |
| 5 | [[../Entities/HDFC Bank Ltd\|HDFC Bank Ltd]] | `HDFCBANK.NS` | อินเดีย | การเงิน | 3.63% |
| 6 | [[../Entities/Larsen & Toubro Ltd\|Larsen & Toubro Ltd]] | `LT.NS` | อินเดีย | อุตสาหกรรม | 3.25% |
| 7 | Bharat Petroleum Corp Ltd | `500547` | - | - | 3.21% |
| 8 | InterGlobe Aviation Ltd | `INDIGO.NS` | อินเดีย | อุตสาหกรรม | 2.96% |
| 9 | Hindustan Petroleum Corp Ltd | `HINDPETRO.NS` | อินเดีย | พลังงาน | 2.96% |
| 10 | HCL Technologies Ltd | `HCLTECH.NS` | อินเดีย | เทคโนโลยี | 2.84% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[MINDIA]] | [[บริษัทหลักทรัพย์จัดการกองทุน เอ็มเอฟซี จำกัด (มหาชน)]] | 6 | 104.04% | 1.64 | **≈ 2.59** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.95%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P00016CTF`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU0946219929:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
