---
title: "CTBC TIP Customized Taiwan Growth and High Dividend ETF"
master_key: "name:ctbc tip customized taiwan growth and high dividend etf"
quote_type: "ETF"
fund_family: "CTBC Securities Investment Tr Co. Ltd."
currency: "TWD"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data, etf]
---

# 🌐 CTBC TIP Customized Taiwan Growth and High Dividend ETF


กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | CTBC TIP Customized Taiwan Growth and High Dividend ETF |
| ISIN | - |
| ประเภท | ETF (จดทะเบียนซื้อขายในตลาด) |
| โครงสร้างทางกฎหมาย | Exchange Traded Fund |
| ประเทศจดทะเบียน | ไต้หวัน |
| บริษัทจัดการ | CTBC Securities Investment Tr Co. Ltd. |
| สกุลเงิน | TWD |
| วันจัดตั้ง | 2023-10-25 |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (Yahoo) | 11.48 พันล้าน TWD |
| Yield | 6.99% |

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 36.39% |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 99.51% |
| เงินสด | 0.49% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| เทคโนโลยี | 59.82% |
| อุตสาหกรรม | 20.00% |
| สินค้าฟุ่มเฟือย | 8.40% |
| สื่อสาร | 5.18% |
| การเงิน | 3.92% |
| วัตถุดิบ | 1.52% |
| สินค้าจำเป็น | 0.76% |
| อสังหาริมทรัพย์ | 0.40% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | กลุ่ม | สัดส่วน |
|---|---|---|---|---|---|
| 1 | [[../Entities/MediaTek Inc\|MediaTek Inc]] | `2454.TW` | ไต้หวัน | เทคโนโลยี | 12.11% |
| 2 | Wiwynn Corp Ordinary Shares | `6669.TW` | ไต้หวัน | เทคโนโลยี | 7.04% |
| 3 | Asustek Computer Inc | `2357.TW` | ไต้หวัน | - | 6.94% |
| 4 | Quanta Computer Inc | `2382.TW` | ไต้หวัน | - | 5.26% |
| 5 | Evergreen Marine Corp (Taiwan) Ltd | `2603.TW` | ไต้หวัน | - | 4.86% |
| 6 | Novatek Microelectronics Corp | `3034.TW` | ไต้หวัน | - | 4.60% |
| 7 | United Integrated Services Co Ltd | `2404.TW` | ไต้หวัน | - | 4.39% |
| 8 | [[../Entities/Realtek Semiconductor Corp\|Realtek Semiconductor Corp]] | `2379.TW` | ไต้หวัน | เทคโนโลยี | 4.19% |
| 9 | AData Technology Co Ltd | `3260.TWO` | ไต้หวัน | - | 3.75% |
| 10 | Largan Precision Co Ltd | `3008.TW` | ไต้หวัน | - | 3.73% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[LHTWGHD]] | [[บริษัทหลักทรัพย์จัดการกองทุน แลนด์ แอนด์ เฮ้าส์ จำกัด]] | 6 | - | 1.74 | - |

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `00934.TW`)
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
