---
title: "iShares STOXX Europe 600 Banks UCITS ETF (DE)"
master_key: "DE000A0F5UJ7"
isin: "DE000A0F5UJ7"
quote_type: "ETF"
fund_family: "BlackRock Asset Management Deutschland AG - ETF"
currency: "EUR"
ongoing_charge: "0.46%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data, etf]
---

# 🌐 iShares STOXX Europe 600 Banks UCITS ETF (DE)

**ISIN:** `DE000A0F5UJ7` · **Ticker:** `EXV1.DE`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | iShares STOXX Europe 600 Banks UCITS ETF (DE) |
| ISIN | `DE000A0F5UJ7` |
| ประเภท | ETF (จดทะเบียนซื้อขายในตลาด) |
| โครงสร้างทางกฎหมาย | Exchange Traded Fund |
| ประเทศจดทะเบียน | เยอรมนี |
| บริษัทจัดการ | BlackRock Asset Management Deutschland AG - ETF |
| สกุลเงิน | EUR |
| วันจัดตั้ง | 2001-04-25 |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (Yahoo) | 3.58 พันล้าน EUR |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.46%** |
| Yield | 3.44% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 23.04% |
| 3 ปี (เฉลี่ยต่อปี) | 47.13% |
| 5 ปี (เฉลี่ยต่อปี) | 32.19% |
| Beta (3 ปี) | 1.08 |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 99.19% |
| เงินสด | 0.81% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| การเงิน | 100.00% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | สัดส่วน |
|---|---|---|---|---|
| 1 | [[../Entities/HSBC Holdings PLC\|HSBC Holdings PLC]] | `HSBA.L` | สหราชอาณาจักร | 14.90% |
| 2 | [[../Entities/Banco Santander SA\|Banco Santander SA]] | `SAN.MC` | - | 8.52% |
| 3 | [[../Entities/Banco Bilbao Vizcaya Argentaria SA\|Banco Bilbao Vizcaya Argentaria SA]] | `BBVA.MC` | - | 6.42% |
| 4 | [[../Entities/UniCredit SpA\|UniCredit SpA]] | `UCG.MI` | อิตาลี | 5.79% |
| 5 | BNP Paribas Act. Cat.A | `BNP.PA` | ฝรั่งเศส | 5.36% |
| 6 | [[../Entities/Intesa Sanpaolo\|Intesa Sanpaolo]] | `ISP.MI` | อิตาลี | 4.73% |
| 7 | [[../Entities/ING Groep NV\|ING Groep NV]] | `INGA.AS` | เนเธอร์แลนด์ | 4.18% |
| 8 | Barclays PLC | `BARC.L` | สหราชอาณาจักร | 3.81% |
| 9 | [[../Entities/Lloyds Banking Group PLC\|Lloyds Banking Group PLC]] | `LLOY.L` | สหราชอาณาจักร | 3.67% |
| 10 | [[../Entities/NatWest Group PLC\|NatWest Group PLC]] | `NWG.L` | สหราชอาณาจักร | 3.17% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[M-EUBANK]] | [[บริษัทหลักทรัพย์จัดการกองทุน เอ็มเอฟซี จำกัด (มหาชน)]] | 7 | 101.82% | 1.44 | **≈ 1.90** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.46%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `EXV1.DE`)
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
