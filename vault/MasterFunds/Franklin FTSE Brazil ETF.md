---
title: "Franklin FTSE Brazil ETF"
master_key: "US35473P8352"
isin: "US35473P8352"
quote_type: "ETF"
category: "Focused Region"
fund_family: "Franklin Templeton Investments"
currency: "USD"
ongoing_charge: "0.19%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data, etf]
---

# 🌐 Franklin FTSE Brazil ETF

**ISIN:** `US35473P8352` · **Ticker:** `FLBR`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | Franklin FTSE Brazil ETF |
| ISIN | `US35473P8352` |
| ประเภท | ETF (จดทะเบียนซื้อขายในตลาด) |
| โครงสร้างทางกฎหมาย | Exchange Traded Fund |
| ประเทศจดทะเบียน | สหรัฐอเมริกา |
| หมวด (Morningstar) | Focused Region |
| บริษัทจัดการ | Franklin Templeton Investments |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2017-11-03 |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (Yahoo) | 558.56 ล้าน USD |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.19%** |
| Yield | 5.70% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 20.15% |
| 3 ปี (เฉลี่ยต่อปี) | 12.54% |
| 5 ปี (เฉลี่ยต่อปี) | 8.23% |
| Beta (3 ปี) | 0.94 |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 98.60% |
| หุ้นบุริมสิทธิ | 0.90% |
| เงินสด | 0.50% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| การเงิน | 26.12% |
| พลังงาน | 19.87% |
| วัตถุดิบ | 15.18% |
| สาธารณูปโภค | 14.10% |
| อุตสาหกรรม | 12.13% |
| สินค้าจำเป็น | 4.31% |
| สินค้าฟุ่มเฟือย | 2.69% |
| สุขภาพ | 2.48% |
| สื่อสาร | 1.62% |
| อสังหาริมทรัพย์ | 0.78% |
| เทคโนโลยี | 0.70% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | กลุ่ม | สัดส่วน |
|---|---|---|---|---|---|
| 1 | [[../Entities/Vale SA\|Vale SA]] | `VALE3.SA` | - | วัสดุ | 10.99% |
| 2 | Itau Unibanco Holding SA Participating Preferred | `ITUB4` | - | - | 9.54% |
| 3 | Petroleo Brasileiro SA Petrobras Participating Preferred | `PETR4` | - | - | 8.13% |
| 4 | Petroleo Brasileiro SA Petrobras | `PETR3.SA` | - | พลังงาน | 7.49% |
| 5 | Bank Bradesco SA Participating Preferred | `BBDC4` | - | - | 3.91% |
| 6 | Itausa Investimentos ITAU SA Participating Preferred | `ITSA4` | - | - | 3.30% |
| 7 | [[../Entities/B3 SA - Brasil Bolsa Balcao\|B3 SA - Brasil Bolsa Balcao]] | `B3SA3.SA` | - | การเงิน | 3.27% |
| 8 | Weg SA | `WEGE3.SA` | - | อุตสาหกรรม | 2.89% |
| 9 | [[../Entities/Axia Energia\|Axia Energia]] | `AXIA3.SA` | - | สาธารณูปโภค | 2.88% |
| 10 | Ambev SA | `ABEV3.SA` | - | ผู้บริโภค | 2.81% |

## คำอธิบายกองทุน (ต้นฉบับภาษาอังกฤษ)

> Under normal market conditions, the fund invests at least 80% of its assets in the component securities of the FTSE Brazil Capped Index and in depositary receipts representing such securities. The FTSE Brazil Capped Index is based on the FTSE Brazil Index and is designed to measure the performance of Brazilian large- and mid-capitalization stocks. The fund is non-diversified.

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[DAOL-BRAZILEQ]] | [[บริษัทหลักทรัพย์จัดการกองทุน ดาโอ จำกัด]] | 6 | 85.00% | 1.96 | **≈ 2.15** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.19%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `FLBR`)
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
