---
title: "DWS Invest Multi Opportunities USD RDMH"
master_key: "LU1433454672"
isin: "LU1433454672"
quote_type: "MUTUALFUND"
category: "USD Flexible Allocation"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "0.64%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 DWS Invest Multi Opportunities USD RDMH

**ISIN:** `LU1433454672` · **Ticker:** `0P00018E6L`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | DWS Invest Multi Opportunities USD RDMH |
| ISIN | `LU1433454672` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | USD Flexible Allocation |
| ผู้จัดการกองทุน | Henning Potstada (เริ่ม 04 Jun 2014) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2014-10-01 |
| นโยบายปันผล | Income |
| สไตล์การลงทุน | Market Cap: Large |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 207.01m GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.64%** |
| Yield | 5.85% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 5.97% |
| 1 ปี (FT) | +8.81% |
| Beta (3 ปี) | 0.86 |
| Morningstar rating | ★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 50.79% |
| ตราสารหนี้ | 29.87% |
| เงินสด | 11.17% |
| อื่น ๆ | 8.06% |
| หุ้นกู้แปลงสภาพ | 0.12% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| เทคโนโลยี | 24.23% |
| การเงิน | 17.50% |
| สุขภาพ | 15.81% |
| อุตสาหกรรม | 14.03% |
| สินค้าฟุ่มเฟือย | 9.72% |
| สื่อสาร | 8.18% |
| สาธารณูปโภค | 5.13% |
| วัตถุดิบ | 2.86% |
| อสังหาริมทรัพย์ | 1.51% |
| สินค้าจำเป็น | 1.01% |
| พลังงาน | 0.02% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | สัดส่วน |
|---|---|---|---|---|
| 1 | [[../Entities/Xtrackers IE Physical Gold ETC\|Xtrackers IE Physical Gold ETC]] | `XGDU.L` | สหราชอาณาจักร | 5.63% |
| 2 | [[../Entities/iShares Core € Corp Bond ETF EUR Dist\|iShares Core € Corp Bond ETF EUR Dist]] | `IEAC.L` | สหราชอาณาจักร | 1.87% |
| 3 | [[../Entities/Xtrackers EUR Corporate Bd ETF 1C\|Xtrackers EUR Corporate Bd ETF 1C]] | `D5BG.DE` | เยอรมนี | 1.32% |
| 4 | [[../Entities/AGSDFUS LX\|AXA SA]] | `CS.PA` | ฝรั่งเศส | 1.17% |
| 5 | [[../Entities/AMAZON.COM INC\|Amazon.com Inc]] | `AMZN` | สหรัฐฯ | 1.14% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[SCBGPLUS]] | [[บริษัทหลักทรัพย์จัดการกองทุน ไทยพาณิชย์ จำกัด]] | 5 | 100.64% | 1.18 | **≈ 1.82** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.64%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P00018E6L`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU1433454672:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
