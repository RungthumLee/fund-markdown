---
title: "KBI Global Energy Transition E USD"
master_key: "IE000JRA46G5"
isin: "IE000JRA46G5"
quote_type: "MUTUALFUND"
category: "Sector Equity Alternative Energy"
domicile: "Ireland"
currency: "USD"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 KBI Global Energy Transition E USD

**ISIN:** `IE000JRA46G5` · **Ticker:** `0P0001PXLO`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | KBI Global Energy Transition E USD |
| ISIN | `IE000JRA46G5` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | Open Ended Investment Company |
| ประเทศจดทะเบียน | Ireland |
| หมวด (Morningstar) | Sector Equity Alternative Energy |
| ผู้จัดการกองทุน | Andros Florides (เริ่ม 08 Apr 2008) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2010-10-08 |
| นโยบายปันผล | Accumulation |
| สไตล์การลงทุน | Market Cap: Mid |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 763.94m GBP |

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 12.17% |
| 1 ปี (FT) | +19.48% |
| Beta (3 ปี) | 1.38 |
| Morningstar rating | ★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 98.41% |
| อื่น ๆ | 1.59% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| เทคโนโลยี | 42.45% |
| อุตสาหกรรม | 33.57% |
| สาธารณูปโภค | 15.47% |
| สินค้าฟุ่มเฟือย | 3.51% |
| การเงิน | 3.39% |
| วัตถุดิบ | 1.61% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | กลุ่ม | สัดส่วน |
|---|---|---|---|---|---|
| 1 | [[../Entities/Infineon Technologies AG\|Infineon Technologies AG]] | `IFX.DE` | เยอรมนี | เทคโนโลยี | 4.17% |
| 2 | [[../Entities/First Solar Inc\|First Solar Inc]] | `FSLR` | สหรัฐฯ | เทคโนโลยี | 3.94% |
| 3 | [[../Entities/Contemporary Amperex Technology Co Ltd Ordinary Shares - Class H\|Contemporary Amperex Technology Co Ltd Ordinary Shares - Class H]] | `03750` | - | - | 3.51% |
| 4 | [[../Entities/HA Sustainable Infrastructure Capital Inc\|HA Sustainable Infrastructure Capital Inc]] | `HASI` | สหรัฐฯ | การเงิน | 3.33% |
| 5 | [[../Entities/NXP SEMICONDUCTORS NV\|NXP Semiconductors NV]] | `NXPI` | สหรัฐฯ | เทคโนโลยี | 3.32% |
| 6 | [[../Entities/Vestas Wind Systems AS\|Vestas Wind Systems AS]] | `VWS.CO` | - | อุตสาหกรรม | 3.19% |
| 7 | [[../Entities/Nextpower Inc Class A\|Nextpower Inc Class A]] | `NXT` | สหรัฐฯ | เทคโนโลยี | 3.15% |
| 8 | [[../Entities/Cadence Design Systems Inc\|Cadence Design Systems Inc]] | `CDNS` | สหรัฐฯ | เทคโนโลยี | 2.71% |
| 9 | [[../Entities/ETN US Equity\|Eaton Corp PLC]] | `ETN` | สหรัฐฯ | อุตสาหกรรม | 2.63% |
| 10 | [[../Entities/SCHNEIDER ELECTRIC SE\|Schneider Electric SE]] | `SU.PA` | ฝรั่งเศส | อุตสาหกรรม | 2.58% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[ES-GGREEN]] | [[บริษัทหลักทรัพย์จัดการกองทุน อีสท์สปริง (ประเทศไทย) จำกัด]] | 7 | 101.05% | 1.73 | - |

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P0001PXLO`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=IE000JRA46G5:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
