---
title: "Brown Advisory Global Leaders B USD Acc"
master_key: "IE00BVVHP563"
isin: "IE00BVVHP563"
quote_type: "MUTUALFUND"
category: "Global Large-Cap Growth Equity"
domicile: "Ireland"
currency: "USD"
ongoing_charge: "0.84%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 Brown Advisory Global Leaders B USD Acc

**ISIN:** `IE00BVVHP563` · **Ticker:** `0P0001C9JR`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | Brown Advisory Global Leaders B USD Acc |
| ISIN | `IE00BVVHP563` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | Open Ended Investment Company |
| ประเทศจดทะเบียน | Ireland |
| หมวด (Morningstar) | Global Large-Cap Growth Equity |
| ผู้จัดการกองทุน | Michael Dillon (เริ่ม 01 May 2015) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2026-02-17 |
| นโยบายปันผล | Accumulation |
| สไตล์การลงทุน | Market Cap: Large |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 4.29bn GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.84%** |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | -1.99% |
| 1 ปี (FT) | +4.89% |
| Beta (3 ปี) | 0.64 |
| Morningstar rating | ★★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 99.59% |
| เงินสด | 0.41% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| การเงิน | 30.93% |
| อุตสาหกรรม | 23.52% |
| เทคโนโลยี | 19.55% |
| สินค้าฟุ่มเฟือย | 8.95% |
| สุขภาพ | 8.62% |
| สื่อสาร | 8.41% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | กลุ่ม | สัดส่วน |
|---|---|---|---|---|---|
| 1 | [[../Entities/MICROSOFT CORP\|Microsoft Corp]] | `MSFT` | สหรัฐฯ | เทคโนโลยี | 7.11% |
| 2 | [[../Entities/Alphabet Inc Class C\|Alphabet Inc Class C]] | `GOOG` | สหรัฐฯ | สื่อสาร | 7.09% |
| 3 | [[../Entities/Mastercard Inc Class A\|Mastercard Inc Class A]] | `MA` | สหรัฐฯ | การเงิน | 5.77% |
| 4 | [[../Entities/Taiwan Semiconductor Manufacturing Co Ltd ADR\|Taiwan Semiconductor Manufacturing Co Ltd ADR]] | `TSM` | สหรัฐฯ | เทคโนโลยี | 5.73% |
| 5 | [[../Entities/Visa Inc Class A\|Visa Inc Class A]] | `V` | สหรัฐฯ | การเงิน | 5.02% |
| 6 | [[../Entities/SAF FP Equity\|Safran SA]] | `SAF.PA` | ฝรั่งเศส | อุตสาหกรรม | 4.68% |
| 7 | [[../Entities/Deutsche Boerse AG\|Deutsche Boerse AG]] | `DB1.DE` | เยอรมนี | การเงิน | 4.67% |
| 8 | Experian PLC | `EXPN.L` | สหราชอาณาจักร | อุตสาหกรรม | 4.13% |
| 9 | [[../Entities/Roche Holding AG Ordinary Shares new\|Roche Holding AG Ordinary Shares new]] | `ROP.SW` | สวิตเซอร์แลนด์ | สุขภาพ | 3.82% |
| 10 | [[../Entities/ASML Holding NV ADR\|ASML Holding NV ADR]] | `ASML` | สหรัฐฯ | เทคโนโลยี | 3.67% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[PRINCIPAL GLEADER]] | [[บริษัทหลักทรัพย์จัดการกองทุน พรินซิเพิล จำกัด]] | 6 | 91.03% | 2.04 | **≈ 2.88** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.84%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P0001C9JR`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=IE00BVVHP563:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
