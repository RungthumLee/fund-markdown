---
title: "Allianz Cyber Security RT USD Acc"
master_key: "LU2286301283"
isin: "LU2286301283"
quote_type: "MUTUALFUND"
category: "Sector Equity Technology"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "1.23%"
feeder_count: 2
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 Allianz Cyber Security RT USD Acc

**ISIN:** `LU2286301283` · **Ticker:** `0P0001LPJM`

กองทุนหลักของกองทุนไทย **2** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | Allianz Cyber Security RT USD Acc |
| ISIN | `LU2286301283` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | Sector Equity Technology |
| ผู้จัดการกองทุน | Erik Swords (เริ่ม 01 Oct 2022) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2025-10-14 |
| นโยบายปันผล | Accumulation |
| สไตล์การลงทุน | Market Cap: Mid |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 363.74m GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **1.23%** |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 23.66% |
| 1 ปี (FT) | +27.49% |
| Beta (3 ปี) | 0.88 |
| Morningstar rating | ★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 96.20% |
| เงินสด | 3.81% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| เทคโนโลยี | 96.95% |
| สื่อสาร | 2.54% |
| อุตสาหกรรม | 0.51% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | กลุ่ม | สัดส่วน |
|---|---|---|---|---|---|
| 1 | [[../Entities/Palo Alto Networks Inc\|Palo Alto Networks Inc]] | `PANW` | สหรัฐฯ | เทคโนโลยี | 10.09% |
| 2 | [[../Entities/BlackBerry Ltd\|BlackBerry Ltd]] | `BB.TO` | แคนาดา | เทคโนโลยี | 8.50% |
| 3 | [[../Entities/CrowdStrike Holdings Inc Class A\|CrowdStrike Holdings Inc Class A]] | `CRWD` | สหรัฐฯ | เทคโนโลยี | 8.07% |
| 4 | [[../Entities/Fortinet Inc\|Fortinet Inc]] | `FTNT` | สหรัฐฯ | เทคโนโลยี | 5.32% |
| 5 | [[../Entities/Cloudflare Inc\|Cloudflare Inc]] | `NET` | สหรัฐฯ | เทคโนโลยี | 4.82% |
| 6 | [[../Entities/Rubrik Inc Class A Shares\|Rubrik Inc Class A Shares]] | `RBRK` | สหรัฐฯ | เทคโนโลยี | 4.73% |
| 7 | [[../Entities/Datadog Inc Class A\|Datadog Inc Class A]] | `DDOG` | สหรัฐฯ | เทคโนโลยี | 4.29% |
| 8 | [[../Entities/CISCO SYSTEMS, INC\|Cisco Systems Inc]] | `CSCO` | สหรัฐฯ | เทคโนโลยี | 4.24% |
| 9 | [[../Entities/Broadcom Inc\|Broadcom Inc]] | `AVGO` | สหรัฐฯ | เทคโนโลยี | 4.18% |
| 10 | [[../Entities/F5 Inc\|F5 Inc]] | `FFIV` | สหรัฐฯ | เทคโนโลยี | 4.12% |

## กองทุนไทยที่ลงทุนในกองนี้

**2 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[KFCYBER]] | [[บริษัทหลักทรัพย์จัดการกองทุน กรุงศรี จำกัด]] | 6 | 100.23% | 1.54 | **≈ 2.77** |
| [[DAOL-CYBERSECURE]] | [[บริษัทหลักทรัพย์จัดการกองทุน ดาโอ จำกัด]] | 7 | 56.45% | 1.96 | **≈ 3.19** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**1.23%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

> [!NOTE]
> กองไทยทั้ง 2 กองนี้ลงทุนในกองทุนหลัก**เดียวกัน** การถือหลายกองจึงไม่ได้กระจายความเสี่ยงอย่างที่คิด

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P0001LPJM`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU2286301283:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
