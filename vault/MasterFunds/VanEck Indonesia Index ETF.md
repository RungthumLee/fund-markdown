---
title: "VanEck Indonesia Index ETF"
master_key: "US92189F8335"
isin: "US92189F8335"
quote_type: "ETF"
category: "Focused Region"
fund_family: "VanEck"
currency: "USD"
ongoing_charge: "0.57%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data, etf]
---

# 🌐 VanEck Indonesia Index ETF

**ISIN:** `US92189F8335` · **Ticker:** `IDX`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | VanEck Indonesia Index ETF |
| ISIN | `US92189F8335` |
| ประเภท | ETF (จดทะเบียนซื้อขายในตลาด) |
| โครงสร้างทางกฎหมาย | Exchange Traded Fund |
| ประเทศจดทะเบียน | สหรัฐอเมริกา |
| หมวด (Morningstar) | Focused Region |
| บริษัทจัดการ | VanEck |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2009-01-15 |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (Yahoo) | 35.31 ล้าน USD |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **0.57%** |
| Yield | 3.16% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | -34.83% |
| 3 ปี (เฉลี่ยต่อปี) | -10.43% |
| 5 ปี (เฉลี่ยต่อปี) | -5.89% |
| Beta (3 ปี) | 0.56 |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 99.91% |
| เงินสด | 0.09% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| การเงิน | 26.72% |
| วัตถุดิบ | 22.56% |
| อุตสาหกรรม | 12.75% |
| พลังงาน | 11.24% |
| สินค้าจำเป็น | 9.69% |
| สื่อสาร | 7.98% |
| สาธารณูปโภค | 3.98% |
| อสังหาริมทรัพย์ | 1.86% |
| สินค้าฟุ่มเฟือย | 1.68% |
| สุขภาพ | 1.55% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | สัดส่วน |
|---|---|---|---|
| 1 | PT Bank Central Asia Tbk | `BBCA.JK` | 8.79% |
| 2 | PT Bank Rakyat Indonesia (Persero) Tbk Registered Shs Series -B- | `BBRI.JK` | 8.01% |
| 3 | PT Bank Mandiri (Persero) Tbk | `BMRI.JK` | 6.55% |
| 4 | PT Telkom Indonesia (Persero) Tbk ADR | `TLK` | 5.93% |
| 5 | PT Astra International Tbk | `ASII.JK` | 5.14% |
| 6 | Jardine Matheson Holdings Ltd | `J36.SI` | 4.83% |
| 7 | PT Amman Mineral Internasional Tbk | `AMMN.JK` | 3.19% |
| 8 | PT Barito Renewables Energy Tbk | `BREN.JK` | 2.72% |
| 9 | PT Bank Negara Indonesia (Persero) Tbk Registered Shs Series -B- | `BBNI.JK` | 2.67% |
| 10 | PT Barito Pacific Tbk | `BRPT.JK` | 2.56% |

## คำอธิบายกองทุน (ต้นฉบับภาษาอังกฤษ)

> The fund normally invests at least 80% of its total assets in securities that comprise the fund's benchmark index. The index includes securities of Indonesian companies. A company is generally considered to be an Indonesian company if it is incorporated in Indonesia or is incorporated outside of Indonesia but has at least 50% of its revenues/related assets in Indonesia. Such companies may include small- and medium-capitalization companies. It is non-diversified.

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[SCBINDO]] | [[บริษัทหลักทรัพย์จัดการกองทุน ไทยพาณิชย์ จำกัด]] | 6 | 101.72% | 1.20 | **≈ 1.77** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**0.57%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `IDX`)
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
