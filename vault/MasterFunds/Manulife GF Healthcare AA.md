---
title: "Manulife GF Healthcare AA"
master_key: "LU0357321016"
isin: "LU0357321016"
quote_type: "MUTUALFUND"
category: "Sector Equity Healthcare"
domicile: "Luxembourg"
currency: "USD"
ongoing_charge: "1.64%"
feeder_count: 1
has_external_data: true
tags: [master-fund, external-data]
---

# 🌐 Manulife GF Healthcare AA

**ISIN:** `LU0357321016` · **Ticker:** `0P0000G25K`

กองทุนหลักของกองทุนไทย **1** กอง · [[../Indexes/master-funds|ดัชนีกองทุนหลักทั้งหมด]] · [[../Concepts/Feeder Fund|Feeder Fund คืออะไร]]

## ข้อมูลกองทุน

| รายการ | ค่า |
|---|---|
| ชื่อกองทุน | Manulife GF Healthcare AA |
| ISIN | `LU0357321016` |
| ประเภท | กองทุนรวม (ไม่ได้จดทะเบียนซื้อขาย) |
| โครงสร้างทางกฎหมาย | SICAV |
| ประเทศจดทะเบียน | Luxembourg |
| หมวด (Morningstar) | Sector Equity Healthcare |
| ผู้จัดการกองทุน | Steven G Slaughter (เริ่ม 05 Apr 2018) |
| สกุลเงิน | USD |
| วันจัดตั้ง | 2015-06-10 |
| นโยบายปันผล | Income |
| สไตล์การลงทุน | Market Cap: Large |

## ขนาดกองทุนและค่าธรรมเนียม

| รายการ | ค่า |
|---|---|
| ขนาดกองทุน (FT) | 297.74m GBP |
| **ค่าธรรมเนียมรวมต่อปี (OCF/TER)** | **1.64%** |
| ค่าธรรมเนียมแรกเข้า (initial charge) | 5.00% |

> [!IMPORTANT]
> ค่าธรรมเนียมนี้เป็นของ **กองทุนหลัก** ผู้ลงทุนไทยจ่าย**ซ้อนกับ**ค่าธรรมเนียมของกองไทยอีกชั้น
> ดู [[../Concepts/ค่าธรรมเนียมกองทุนรวม|ค่าธรรมเนียมกองทุนรวม]]

## ผลการดำเนินงาน

| ช่วงเวลา | ผลตอบแทน |
|---|---|
| YTD | 3.14% |
| 1 ปี (FT) | +26.35% |
| Beta (3 ปี) | 0.96 |
| Morningstar rating | ★★★★ |

> ตัวเลขจากแหล่งภายนอก อาจคิดคนละสกุลเงินและคนละวันอ้างอิงกับ NAV ของกองไทย · ผลตอบแทนในอดีตไม่รับประกันอนาคต

## สัดส่วนประเภทสินทรัพย์

| ประเภท | สัดส่วน |
|---|---|
| หุ้น | 98.31% |
| เงินสด | 1.69% |

## สัดส่วนกลุ่มอุตสาหกรรม

| กลุ่มอุตสาหกรรม | สัดส่วน |
|---|---|
| สุขภาพ | 100.00% |

## หลักทรัพย์ที่ถือมากที่สุด

นี่คือ **look-through** ที่แท้จริง — สิ่งที่เงินของผู้ลงทุนไทยไปลงทุนจริง

| # | หลักทรัพย์ | Ticker | ตลาด | กลุ่ม | สัดส่วน |
|---|---|---|---|---|---|
| 1 | [[../Entities/Eli Lilly and Co\|Eli Lilly and Co]] | `LLY` | สหรัฐฯ | สุขภาพ | 10.05% |
| 2 | [[../Entities/Johnson & Johnson\|Johnson & Johnson]] | `JNJ` | สหรัฐฯ | สุขภาพ | 6.98% |
| 3 | [[../Entities/AstraZeneca PLC\|AstraZeneca PLC]] | `AZN.L` | สหราชอาณาจักร | สุขภาพ | 6.04% |
| 4 | [[../Entities/AbbVie Inc\|AbbVie Inc]] | `ABBV` | สหรัฐฯ | สุขภาพ | 5.61% |
| 5 | [[../Entities/Amgen Inc\|Amgen Inc]] | `AMGN` | สหรัฐฯ | สุขภาพ | 5.00% |
| 6 | [[../Entities/Medtronic PLC\|Medtronic PLC]] | `MDT` | สหรัฐฯ | สุขภาพ | 3.65% |
| 7 | [[../Entities/Roche Holding AG Ordinary Shares new\|Roche Holding AG Ordinary Shares new]] | `ROP.SW` | สวิตเซอร์แลนด์ | สุขภาพ | 3.51% |
| 8 | [[../Entities/BrightSpring Health Services Inc\|BrightSpring Health Services Inc]] | `BTSG` | สหรัฐฯ | สุขภาพ | 3.20% |
| 9 | [[../Entities/Chugai Pharmaceutical Co Ltd\|Chugai Pharmaceutical Co Ltd]] | `4519.T` | ญี่ปุ่น | สุขภาพ | 2.89% |
| 10 | [[../Entities/Vertex Pharmaceuticals Inc\|Vertex Pharmaceuticals Inc]] | `VRTX` | สหรัฐฯ | สุขภาพ | 2.83% |

## กองทุนไทยที่ลงทุนในกองนี้

**1 กอง** — เรียงตามสัดส่วนที่ถือ

| กองทุนไทย | บลจ. | เสี่ยง | % NAV ที่ถือกองนี้ | TER ไทย (%) | รวม 2 ชั้น ≈ (%) |
|---|---|---|---|---|---|
| [[FP HCARE]] | [[บริษัทหลักทรัพย์จัดการกองทุน เฟิร์ส พลัส (ประเทศไทย) จำกัด]] | 7 | 99.37% | 2.17 | **≈ 3.81** |

> **รวม 2 ชั้น** = TER ของกองไทย + OCF ของกองหลัก (**1.64%**) — ค่าธรรมเนียมที่แท้จริงที่ผู้ลงทุนไทยแบกทั้งหมด · ดู [[../Concepts/ค่าธรรมเนียมสองชั้นของ Feeder Fund|ค่าธรรมเนียมสองชั้น]]

---

## แหล่งข้อมูล

- **Yahoo Finance** — โปรไฟล์ ผลตอบแทน sector holdings (symbol `0P0000G25K`)
- **FT.com** — ค่าธรรมเนียม ขนาดกองทุน domicile ผู้จัดการ ([tearsheet](https://markets.ft.com/data/funds/tearsheet/summary?s=LU0357321016:USD))
- **ก.ล.ต. ไทย** — รายชื่อกองทุนไทยที่ลงทุนและสัดส่วนที่ถือ

[[../Indexes/00-home|🏠 Home]] · [วิธีเก็บข้อมูลกองทุนหลัก](../../docs/guides/master-fund-sources.md)
