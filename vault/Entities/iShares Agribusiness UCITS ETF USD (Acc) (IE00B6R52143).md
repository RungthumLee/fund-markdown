---
title: "iShares Agribusiness UCITS ETF USD (Acc)"
entity_id: "isin:IE00B6R52143"
isin: "IE00B6R52143"
ticker: "IS0C"
figi: "BBG002TWWJ97"
share_class_figi: "BBG0022QWZN7"
figi_type: "ETP"
kind: "fund"
domicile_country: "ไอร์แลนด์"
market_country: "สหราชอาณาจักร"
country: "สหราชอาณาจักร"
fund_count: 2
indirect_fund_count: 0
alias_count: 5
tags: [entity, entity-fund, multi-alias]
---

# 📦 iShares Agribusiness UCITS ETF USD (Acc)

**ประเภท:** หน่วยลงทุน · **ISIN:** `IE00B6R52143` · **Ticker:** `IS0C (GR)`

**ตลาดซื้อขาย:** สหราชอาณาจักร · **จดทะเบียน:** ไอร์แลนด์ จาก ISIN

ถือโดยตรง **2** กอง

[[../Indexes/by-holding|ดัชนีสินทรัพย์]] · [[../Indexes/by-lookthrough|ดัชนีการถือทางอ้อม]] · [[../Concepts/การรวมชื่อสินทรัพย์|ชื่อนี้รวมมาจากไหน]] · [[../Concepts/Look-through การถือทางอ้อม|Look-through คืออะไร]]

## ชื่อที่พบในข้อมูลดิบ

ข้อมูล ก.ล.ต. สะกดสินทรัพย์นี้ **5 แบบ** ซึ่งถูกรวมเป็นรายการเดียวแล้ว

> [!NOTE] ถ้าไม่รวม ตัวเลขการกระจุกตัวจะต่ำกว่าความจริง เพราะฐานะเดียวถูกนับแยกเป็นหลายรายการ

`BLACKROC` · `ISAG LN` · `ISAGLN` · `iShares Agribusiness UCITS ETF`

## กองทุนไทยที่ถือโดยตรง (2 กอง)

| กองทุน | บลจ. | % NAV | นโยบาย |
|---|---|---|---|
| [[KT-AGRIANDFOOD\|KT-AGRIANDFOOD]] | บริษัทหลักทรัพย์จัดการกองทุน กรุงไทย จำกัด (มหาชน) | 98.91% | ตราสารทุน |
| [[DAOL-AGRI\|DAOL-AGRI]] | บริษัทหลักทรัพย์จัดการกองทุน ดาโอ จำกัด | 96.69% | ตราสารทุน |

> น้ำหนักที่แสดงคือ **% NAV สูงสุด** ที่กองนั้นเคยรายงานถือสินทรัพย์นี้ ในงวดที่มีข้อมูล

## รหัสอ้างอิงสากล

| รายการ | ค่า |
|---|---|
| FIGI | `BBG002TWWJ97` |
| Share Class FIGI | `BBG0022QWZN7` |
| ประเภทตาม Bloomberg | ETP |
| หมวดตลาด | Equity |

ที่มา: OpenFIGI ของ Bloomberg · [วิธีใช้และข้อจำกัด](../../docs/guides/openfigi.md)

[[../Indexes/00-home|🏠 Home]] · [[../Indexes/by-holding|ดัชนีสินทรัพย์]]
